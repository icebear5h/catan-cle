"""Mutable board state: buildings, roads, and cached buildability queries."""
...
from cle.game_engine.models.board.graph import STATIC_GRAPH, NodeGraph, sorted_edge
from cle.game_engine.models.board.roads import (
    RoadState,
    longest_acyclic_path,
    road_components_and_lengths,
)
from cle.game_engine.models.enums import CITY, SETTLEMENT, FastBuildingType, FastResource
...
class Board:
    ...
    # Keep the historical pickle path now that board is a package.
    __module__ = "cle.game_engine.models.board"

    def __init__(self, catan_map: CatanMap | None = None, initialize: bool = True) -> None:
        self.buildable_subgraph: NodeGraph | None = None
        self.buildable_edges_cache: dict[Color, list[EdgeId]] = {}
        self.player_port_resources_cache: dict[Color, set[FastResource | None]] = {}
        if initialize:
            ...
            self.roads: dict[EdgeId, Color] = dict()  # (node_id, node_id) => color

            # color => int{}[] (list of node_id sets) one per component
            #   nodes in sets are incidental (might not be owned by player)
            self.connected_components: defaultdict[Color, list[set[NodeId]]] = defaultdict(list)
            self.board_buildable_ids: set[NodeId] = set(self.map.land_nodes)
            self.road_lengths: defaultdict[Color, int] = defaultdict(int)
            self.road_color: Color | None = None
            self.road_length: int = 0

            # assumes there is at least one desert:
            self.robber_coordinate: Coordinate = filter(
    ...
    def build_settlement(
        self, color: Color, node_id: NodeId, initial_build_phase: bool = False
    ) -> RoadState:
    ...
    def dfs_walk(self, node_id: NodeId, color: Color) -> set[NodeId]:
    ...
    def _get_connected_component_index(self, node_id: NodeId, color: Color) -> int | None:
        for i, component in enumerate(self.connected_components[color]):
            if node_id in component:
                return i
        return None

    def build_road(self, color: Color, edge: EdgeId) -> RoadState:
    ...
    def recompute_road_state(self) -> RoadState:
        ...
        components, lengths = road_components_and_lengths(self)
    ...
    def build_city(self, color: Color, node_id: NodeId) -> None:
    ...
    def buildable_node_ids(self, color: Color, initial_build_phase: bool = False) -> list[NodeId]:
        if initial_build_phase:
            return sorted(list(self.board_buildable_ids))

        subgraphs = self.find_connected_components(color)
        nodes = set[NodeId]().union(*subgraphs)
        return sorted(list(nodes.intersection(self.board_buildable_ids)))

    def buildable_edges(self, color: Color) -> list[EdgeId]:
        ...
        assert self.buildable_subgraph is not None  # set by __init__ or copy()
        candidate_edges = self.buildable_subgraph.edges(expandable_nodes)
        for edge in candidate_edges:
            if self.get_edge_color(edge) is None:
                expandable.add(sorted_edge(edge))

        self.buildable_edges_cache[color] = list(expandable)
        return self.buildable_edges_cache[color]

    def get_player_port_resources(self, color: Color) -> set[FastResource | None]:
        ...
        resources: set[FastResource | None] = set()
    ...
    def find_connected_components(self, color: Color) -> list[set[NodeId]]:
        """
        Returns:
    ...
    def continuous_roads_by_player(self, color: Color) -> list[list[EdgeId]]:
        paths = []
    ...
    # ===== Helper functions
    def get_node_color(self, node_id: NodeId) -> Color | None:
    ...
    def get_edge_color(self, edge: EdgeId) -> Color | None:
        # using try-except instead of .get for performance
        try:
