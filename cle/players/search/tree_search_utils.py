...
from game_engine.models.map import number_probability
...
from game_engine.features import build_production_features
from cle.players.search.value import value_production
...
def execute_deterministic(game, action):
    copy = game.copy()
    copy.step(action, validate_action=False)
    return [(copy, 1)]
...
def execute_spectrum(game, action):
    ...
    if action.action_type in DETERMINISTIC_ACTIONS:
        ...
        for card in set(current_deck):
            ...
            try:
                option_game.step(option_action, validate_action=False)
            except Exception:
