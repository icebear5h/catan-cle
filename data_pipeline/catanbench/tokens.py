...
from engine.models.enums import ActionType, RESOURCES, CITY, ROAD, SETTLEMENT
...
def token_specs() -> List[CatanTokenSpec]:
    ...
    specs.extend(
        CatanTokenSpec(node_token(node_id), "node", node_id)
        for node_id in range(NUM_NODES)
    )
    specs.extend(
        CatanTokenSpec(edge_token(edge), "edge", list(edge))
        for edge in base_edges()
    )
    specs.extend(
        CatanTokenSpec(tile_token(tile_id), "tile", tile_id)
        for tile_id in range(NUM_TILES)
    )
    specs.extend(
        CatanTokenSpec(port_token(port_id), "port", port_id)
    ...
    specs.extend(
        CatanTokenSpec(color_token(color), "color", color.value)
        for color in Color
    )
    specs.extend(
