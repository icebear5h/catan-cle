...
class ActionPrompt(Enum):
    ...
    STEAL = "STEAL"  # After moving robber, choose who to steal from
    DECIDE_TRADE = "DECIDE_TRADE"
    DECIDE_ACCEPTEES = "DECIDE_ACCEPTEES"
    DECIDE_COUNTER_OFFERS = "DECIDE_COUNTER_OFFERS"  # Original offerer responds to counter offers
...
class ActionType(Enum):
    ...
    MOVE_ROBBER = "MOVE_ROBBER"  # value is coordinate tuple. Separate from STEAL.
    STEAL = "STEAL"  # value is (Color, resource|None). Resource is card stolen.
    ...
    # COUNTER_OFFER value is 10-resource tuple (same as OFFER_TRADE) - a counter proposal
    #   from a non-turn player, directed at the turn player
    COUNTER_OFFER = "COUNTER_OFFER"
    # JOIN_COUNTER_OFFER value is the color of the counter-offer creator. Indicates this
    #   player would also make the same offer to the turn player.
    JOIN_COUNTER_OFFER = "JOIN_COUNTER_OFFER"
    # CONFIRM_TRADE value is the acceptee_color (who to trade with)
    CONFIRM_TRADE = "CONFIRM_TRADE"
    # ACCEPT_COUNTER_OFFER value is counter_creator_color, or (counter_creator_color, trade_partner_color)
    #   to pick a specific partner from the counter's acceptees
