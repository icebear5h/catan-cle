...
def render_variant(
    ...
    *,
    ...
    for row in rows:
        contract = read_json_object(source_dir / text(row["contract_path"], "contract_path"))
        image = render_contract_image(contract, image_size=image_size, style=style)
        image_path = output_dir / text(row["image_path"], "image_path")
        image_path.parent.mkdir(parents=True, exist_ok=True)
        image.save(image_path)

    source_metadata = read_json_object(source_dir / "metadata.json")
    metadata: JsonDict = {
        **source_metadata,
