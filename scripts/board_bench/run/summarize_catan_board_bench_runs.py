    ...
    with path.open() as handle:
        for line_number, line in enumerate(handle, start=1):
            if not line.strip():
                continue
            try:
                yield obj(json.loads(line), f"{path}:{line_number}")
...
def load_run(run_dir: Path) -> list[JsonDict]:
    ...
    question_ids = [text(record["question_id"], "question_id") for record in records]
    duplicates = sorted(
...
def nested_usage_value(usage: JsonDict, section: str, key: str) -> int:
    ...
    return int(numeric(value or 0, f"{section}.{key}"))


def _usage_of(record: JsonDict) -> JsonDict:
    usage = record.get("usage") or {}
    return usage if isinstance(usage, dict) else {}


def _score_of(record: JsonDict) -> JsonDict:
    return obj(record["score"], "record score")


def summarize_records(records: Sequence[JsonDict]) -> JsonDict:
    successful = [record for record in records if not record.get("error")]
    exact_correct = sum(bool(_score_of(record)["correct"]) for record in successful)
    component_correct = sum(
        integer(_score_of(record)["component_correct"], "component_correct")
        for record in successful
    )
    component_total = sum(
        integer(_score_of(record)["component_total"], "component_total")
        for record in successful
    )
    latencies = [
        integer(record["latency_ms"], "latency_ms")
        for record in successful
    ...
    for record in successful:
        ...
        for key in ("prompt_tokens", "completion_tokens", "total_tokens"):
            usage_totals[key] += int(numeric(usage.get(key, 0) or 0, key))
    ...
    for category in sorted({text(record["category"], "category") for record in records}):
        category_records = [record for record in successful if record["category"] == category]
        category_exact = sum(
            bool(_score_of(record)["correct"]) for record in category_records
        )
    ...
    return {
        ...
        "usage": {key: total for key, total in sorted(usage_totals.items())},
        "models": list(sorted({str(record.get("model_id")) for record in records})),
        "served_models": list(sorted({str(record.get("served_model")) for record in successful})),
        "providers": list(sorted({str(record.get("provider")) for record in successful})),
        "categories": {name: summary for name, summary in categories.items()},
    }
...
def exact_two_sided_binomial_p(first_only: int, second_only: int) -> float | None:
    ...
    probability: float = min(1.0, 2.0 * tail / (2**discordant))
    return probability
...
def paired_exact(
    ...
    first_by_id = {
        text(record["question_id"], "question_id"): record
    ...
    for question_id in question_ids:
        ...
        pairs.append(
            (bool(_score_of(first)["correct"]), bool(_score_of(second)["correct"]))
        )
