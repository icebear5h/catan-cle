...
from scripts.board_bench.shapes import JsonDict, numeric, obj, read_json_object, text
    ...
    return {
        ...
        "cohorts": {
            "perception_110": {
                ...
                "models": {name: row for name, row in perception.items()},
            },
            "strict_raw_vision_60": {
                ...
                "models": {name: row for name, row in strict_image.items()},
            },
            "strict_text_60": {
                "questions": 60,
                "scoring": "strict_typed_json/v2",
                "format": "indexed_tile_rows",
                "models": {name: row for name, row in strict_text.items()},
            },
        },
        "matched_qwen_max_image_vs_text": matched_qwen,
        "artifact_digest": json_digest(
            {
                "perception": {name: row for name, row in perception.items()},
                "strict_image": {name: row for name, row in strict_image.items()},
                "strict_text": {name: row for name, row in strict_text.items()},
...
def load_strict_run(run_dir: Path, *, expected_records: int) -> list[JsonDict]:
    ...
    for row in records:
        ...
        copy["score"] = score_strict_json_answer(
            obj(row["expected"], "expected answer"), text(row["response"], "response")
        )
...
def summarize_strict_run(run_dir: Path, records: Sequence[JsonDict]) -> JsonDict:
    ...
    overall = obj(summary["overall"], "summary overall")
    exact = sum(bool(_score(row)["correct"]) for row in records)
    json_valid = sum(bool(_score(row)["json_valid"]) for row in records)
    if overall["exact"] != exact or overall["json_valid"] != json_valid:
        raise ValueError(f"stored strict summary is stale: {run_dir}")
    return {
        "run_dir": str(run_dir),
        "model": next(iter({text(row["model_id"], "model_id") for row in records})),
        ...
        "median_latency_ms": overall["median_latency_ms"],
        "providers": overall["providers"],
        "categories": {
            category: {
                "requests": obj(values, "category summary")["requests"],
                "exact": obj(values, "category summary")["exact"],
                "exact_accuracy": obj(values, "category summary")["exact_accuracy"],
                "json_valid": obj(values, "category summary")["json_valid"],
            }
            for category, values in sorted(obj(summary["categories"], "categories").items())
        },
    }


def validate_current_model_records(records: Sequence[JsonDict]) -> None:
    for model_id in {str(row.get("model_id", "")) for row in records}:
...
def paired_modalities(
    *,
    ...
    image_by_id = {text(row["question_id"], "question_id"): row for row in image_records}
    text_by_id = {text(row["question_id"], "question_id"): row for row in text_records}
    ...
    for question_id in question_ids:
        image_row = image_by_id[question_id]
        text_row = text_by_id[question_id]
        if image_row["category"] != text_row["category"]:
            raise ValueError(f"category differs across modalities: {question_id}")
        pairs.append(
            (bool(_score(image_row)["correct"]), bool(_score(text_row)["correct"]))
        )
