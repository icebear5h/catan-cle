"""Build leakage-control and pre-SFT baseline artifacts for CatanBench."""
...
BENCH_DIR = ROOT / "data_pipeline/catanbench/datasets/catanbench_100"
MANIFEST_PATH = BENCH_DIR / "manifest.jsonl"
METADATA_PATH = BENCH_DIR / "metadata.json"
OPENROUTER_EVAL_DIR = BENCH_DIR / "openrouter_eval"
RAW_REPLAY_DIR = ROOT / "data_pipeline/bootstrapping/data/raw_replays"
SCRAPER_DIR = ROOT / "data_pipeline/bootstrapping/scrapers"

LEAKAGE_DIR = BENCH_DIR / "leakage"
RESULTS_DIR = BENCH_DIR / "reports"

BENCHMARK_IDS_JSON = LEAKAGE_DIR / "benchmark_game_ids.json"
BENCHMARK_IDS_MD = LEAKAGE_DIR / "benchmark_game_ids.md"
TRAINING_CANDIDATES_JSON = SCRAPER_DIR / "4p_games_training_candidates.json"
BASELINE_TABLE_MD = RESULTS_DIR / "catanbench_100_openrouter_baseline.md"

INDEX_FILES = [
    SCRAPER_DIR / "4p_games_top100.json",
    SCRAPER_DIR / "4p_games_current.json",
    SCRAPER_DIR / "4p_games_me_all.json",
]
...
def build_training_candidates(benchmark_ids: set[str]) -> tuple[list[dict[str, Any]], dict[str, int]]:
    seen: set[str] = set()
...
def render_leakage_md(
    ...
    return "\n\n".join(
        [
            ...
            (
                f"Benchmark: {metadata.get('name', 'CatanBench-100')} with "
...
def main() -> None:
    ...
    write_json(
        BENCHMARK_IDS_JSON,
        {
            "schema": "catanbench/leakage/v0",
            "benchmark": metadata.get("name", "CatanBench-100"),
