#!/usr/bin/env python3
"""Plan, run, or verify GPT narrator assembly by causal observation."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Assemble grounded narrator reasoning by decision/observation."
    )
    ...
    for command in ("plan", "run"):
        ...
        if command == "run":
            ...
            command_parser.add_argument("--max-tokens", type=int, default=2000)
...
def main() -> None:
    ...
    scan = scan_observation_jobs(args.game_id, args.decision_artifact_dir)
    plan = build_generation_plan(scan, args.model)
    if args.command == "plan":
        _write_plan(args.output_dir / "plan.json", plan)
        print(
            json.dumps(
                {
