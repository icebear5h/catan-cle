#!/usr/bin/env python
"""Evaluate OpenRouter VLMs on CatanBench engine-scored QA.
...
from catanbench.scoring import PROBE_CATEGORIES, PROBE_SYSTEM_PROMPT

load_dotenv()


BENCH_DIR = Path("data_pipeline/catanbench/datasets/catanbench_100")
QUESTION_DIR = BENCH_DIR / "questions"
PROBE_DIR = Path("sft/data/isolated_visuals")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--suite", choices=["catanbench", "probe"], default="catanbench")
    ...
    parser.add_argument(
        "--provider",
        default="openrouter",
        choices=["openrouter", "moondream"],
        help="Provider to call: openrouter (default) or moondream.",
    )
...
def main() -> None:
    ...
    question_dir = args.question_dir or (PROBE_QUESTION_DIR if args.suite == "probe" else QUESTION_DIR)
    categories = split_csv(args.categories) if args.categories else (
        list(PROBE_CATEGORIES) if args.suite == "probe" else list(DEFAULT_CATEGORIES)
    )
    ...
    output_dir = args.output_dir or bench_dir / "openrouter_eval" / timestamp_slug()
    ...
    if not api_key:
        env_var = "MOONDREAM_API_KEY" if args.provider == "moondream" else "OPENROUTER_API_KEY"
        raise SystemExit(
    ...
    with response_path.open("w") as out:
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.concurrency) as pool:
            futures = {
                pool.submit(
                    call_moondream if args.provider == "moondream" else call_openrouter,
                    api_key,
                    job["model_id"],
    ...
    if isinstance(value, dict):
        for key in (
            ...
            if key in value:
                ...
                if text:
                    return text
        candidates = [extract_content(v) for v in value.values() if isinstance(v, (str, list, dict))]
...
def model_supports_reasoning_control(model_id: str) -> bool:
    # Qwen 3.5 variants are reasoning-first by default in some OpenRouter routes.
    # Passing explicit reasoning control avoids empty final content in practice.
    model_id_lower = model_id.lower()
    return "qwen3.5" in model_id_lower or "qwen3.6" in model_id_lower
...
def attach_contract_if_available(qa: JsonDict, bench_dir: Path, contracts: Dict[str, JsonDict]) -> None:
...
def local_atlas_context(qa: JsonDict) -> str:
    ...
    if category == "node_occupancy":
        ...
        if node:
            port_text = f" ports {' '.join(node['port_tokens'])}" if node["port_tokens"] else " no ports"
            return (
...
def call_openrouter(
    ...
    *,
    ...
    headers = {
        ...
        "X-Title": "CatanBench OpenRouter Eval",
    }
...
def call_moondream(
    ...
    *,
    ...
    def run_with_optional_resolve(resolve_ip: Optional[str] = None) -> subprocess.CompletedProcess[str]:
        cmd = [
            "curl",
            "-sS",
            "-m",
...
def score_answer(qa: JsonDict, response: str) -> JsonDict:
    ...
    if category == "robber_resource_number":
        ...
        number = target["number"]
        number_ok = "NO_NUMBER" in normalized if number is None else str(number) in number_tokens(normalized)
    ...
    if category == "edge_road_owner":
        ...
        if road_color is None:
            return component_score("EMPTY" in normalized)
        return component_score(contains_value(normalized, color_token(target, color_key="road_color")))
...
def contains_count(
    ...
    return contains_labeled_count(normalized_response, labels, expected_count) or contains_bare_count(
        normalized_response,
        expected_count,
    )
...
def summarize_records(records: Sequence[JsonDict]) -> JsonDict:
    ...
    latencies = [record["latency_ms"] for record in attempted if record.get("latency_ms") is not None]
    return {
