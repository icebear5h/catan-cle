...
```text
sft/configs/renderer_style_current.json
```
...
```text
sft/data/synthetic_node_factors/messages_with_images.jsonl
sft/data/synthetic_node_factors/render_metadata.json
```
...
```text
sft/data/catan_atlas_topology.jsonl
390 rows
```
...
```text
sft/data/modal_vlm_smoke.jsonl
24 rows
...
Run the 8B QLoRA visual smoke first:
...
the top-level catanbench compatibility package. Fixed the wrapper to import
data_pipeline.catanbench.tokens directly.
HF Hub ran unauthenticated, so downloads may be slower or rate-limited.
