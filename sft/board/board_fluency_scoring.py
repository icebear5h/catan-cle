...
from sft.board.symbolic_board_tasks import strict_json
...
def _resource_vector(value: object, *, signed: bool = False) -> None:
    ...
    _require(all(type(n) is int and (signed or n >= 0)
                 for n in cast("JsonDict", value).values()),
             "resource counts must be integers (nonnegative outside delta)")


def _json_answer(text: str, operation: str) -> JsonDict:
    decoded = strict_json(text)
    if operation == "local_node_tiles":
        _require(type(decoded) is dict and 1 <= len(cast("JsonDict", decoded)) <= 3
                 and cast("JsonDict", decoded).keys() <= _atlas_vocabulary("T"),
                 "invalid local tile keys")
        for entry in cast("JsonDict", decoded).values():
            _keys(entry, {"resource", "number"})
            tile = cast("JsonDict", entry)
        ...
        _resource_vector(decoded)
    elif operation == "road_removal_connectivity":
        _keys(decoded, {"before", "after"})
        _require(all(n is None or (type(n) is int and n >= 0)
                     for n in cast("JsonDict", decoded).values()),
                 "distances must be nonnegative integers or null")
    else:
        _keys(decoded, {"before", "after", "delta"})
        for key, vector in cast("JsonDict", decoded).items():
...
def _typed_equal(expected: object, actual: object) -> bool:
    ...
    if isinstance(expected, dict):
        other = cast("dict[str, object]", actual)
        return expected.keys() == other.keys() and all(
            _typed_equal(value, other[key]) for key, value in expected.items()
        )
    if isinstance(expected, list):
        items = cast("list[object]", actual)
        return len(expected) == len(items) and all(
            _typed_equal(a, b) for a, b in zip(expected, items)
        )
...
def validate_board_fluency_metadata(metadata: JsonDict) -> None:
    ...
    _require(metadata.get("family") == OPERATION_FAMILY[cast("str", operation)]
    ...
    if schema == SCHEMA:
        ...
        _require(isinstance(provenance, dict) and provenance.get("split") == split
                 ...
                 and cast("JsonDict", provenance["source"]).get("split") == split,
                 "conflicting SFT source split")
    ...
    _require(isinstance(target, dict) and set(target) == {"state", "query"}
             ...
             and target["query"].get("operation") == operation,
             "invalid board-fluency target operation")


def score_board_fluency(expected: str, response: str,
    ...
    operation = as_str(metadata["operation"])
