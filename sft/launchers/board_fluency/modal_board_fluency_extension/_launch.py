...
MODAL_PROFILE=icebear5h CATAN_HF_SECRET_NAME=huggingface-secret-2     .venv/bin/python -B -m sft.launchers.board_fluency.modal_board_fluency_extension     --run-name board-fluency-extension-20260915-r01 --budget-usd 15
...
def launch(run_name: str, budget_usd: float = 15, execute: bool = False) -> JsonLikeDict:
    ...
    return {**state, "local_receipt": str(local / "orchestration.json"),
            "stop": f"MODAL_PROFILE=icebear5h {sys.executable} -B -m sft.launchers.board_fluency.modal_board_fluency_extension --run-name {run_name} --stop"}
