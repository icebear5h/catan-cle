...
from sft.launchers.board_fluency.modal_board_fluency_extension._types import at
...
def coordinate(plan: JsonDict, absolute_deadline: float, app_id: str) -> JsonLikeDict:
    try:
...
def coordinate_work(plan: JsonDict, absolute_deadline: float, app_id: str) -> JsonLikeDict:
    original.sft_runs.reload()
    root = Path(as_str(plan["root"]))
    ...
    # `stages` is the same object as `result["stages"]`; `completed` keeps each
    # stage's typed receipt for the comparison below.
    stages: dict[str, JsonLikeDict] = {}
    completed: dict[str, JsonDict] = {}
    result: JsonLikeDict = {"status": "running", "launch_sha256": shared.digest(plan), "started_at": shared.now(),
    ...
    try:
        ...
        for phase, function in (("prepare", prepare_cpu), ("train", train_h200), ("posteval", posteval_h200)):
            ...
            entry: JsonLikeDict = {"status": "starting", "started_at": shared.now(),
        ...
        before = as_dict(at(completed["prepare"], "result", "baselines"))
        after = as_dict(at(completed["posteval"], "result", "panels"))
        if any(at(before, panel, "rows") != at(after, panel, "rows") for panel in before):
            raise ValueError("comparison requires complete identical panel coverage")
        result.update(status="completed", comparison={panel: {
            "before": at(before, panel, "correct"), "after": at(after, panel, "correct"),
            "rows": at(before, panel, "rows"), "baseline_complete": True,
            "before_files": at(before, panel, "files"), "after_files": at(after, panel, "files"),
        ...
        if phase is not None and phase in stages:
            stages[phase].update(status="failed", ended_at=shared.now(), error=result["error"])
...
def stop_run(run_name: str) -> JsonDict:
    ...
    original.stop_app(as_str(state["app_id"]))
