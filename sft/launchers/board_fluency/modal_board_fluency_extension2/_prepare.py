    ...
    def on_train_begin(
        ...
        **kwargs: object,
        ...
        for split, path, rows, expected in (
            ("train", self.config.train_jsonl, 2048, at(self.prepared, "suffix", "sha256")),
...
def prepare_work(plan: JsonDict, deadline: float) -> JsonLikeDict:
    ...
    lengths = {"train_suffix": ext1.text_boundaries(
        tokenizer, [as_dict(loads_json(line)) for line in suffix.splitlines()],
