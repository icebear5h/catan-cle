...
from ._prepare import Extension2Callback, prepare_work, prepared_for, verify_initialization
...
def train_work(plan: JsonDict, deadline: float) -> JsonLikeDict:
    ...
    return {"training": result, "final_checkpoint": str(final), "committed_steps": callback.saved_steps,
            ...
            "consumed": at(plan, "suffix", "consumed"), "fresh": at(plan, "suffix", "fresh"),
            "repeat": at(plan, "suffix", "repeat"),
...
def posteval_work(plan: JsonDict, deadline: float) -> JsonLikeDict:
    ...
    checkpoint = as_str(at(plan, "config", "output_dir")) + "/checkpoints/checkpoint-256"
...
def bounded_stage(plan: JsonDict, stage: str, deadline: float) -> JsonDict:
    ...
    command = [sys.executable, "-B", "-m", "sft.launchers.board_fluency.modal_board_fluency_extension2", "--worker", stage,
