...
MODAL_PROFILE=icebear5h CATAN_HF_SECRET_NAME=huggingface-secret-2     .venv/bin/python -B -m sft.launchers.board_fluency.modal_board_fluency_extension3     --run-name board-fluency-extension-20260915-r03 --budget-usd 31
...
from ._types import recorded_app_id


def launch(run_name: str, budget_usd: float = 31, execute: bool = False) -> JsonDict:
    ...
    state: JsonDict = {"status": "reserving", "launch_sha256": shared.digest(plan)}
        ...
        with app.run(detach=True):
            ...
            try:
                ...
                with deadline_alarm(absolute_deadline):
                    call = coordinate.spawn(plan, absolute_deadline, recorded_app_id(app_id))
                ...
                try:
                    original.stop_app(recorded_app_id(app_id))
    ...
    return {**state, "local_receipt": str(local / "orchestration.json"),
            "stop": f"MODAL_PROFILE=icebear5h {sys.executable} -B -m sft.launchers.board_fluency.modal_board_fluency_extension3 --run-name {run_name} --stop"}
