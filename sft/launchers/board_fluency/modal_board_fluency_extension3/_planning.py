...
def budget_plan(budget_usd: float = 31) -> JsonLikeDict:
...
def read_parent(root: Path) -> dict[str, JsonDict]:
    ...
    if (config["max_steps"] != 256 or config["save_total_limit"] != 8
            or config["save_steps"] != 32 or config["eval_steps"] != 32
            or config["initial_bundle"] != R01_CHECKPOINT
            or config["output_dir"] != PARENT_ROOT + "/training"
            or config["train_jsonl"] != PARENT_ROOT + "/prepare/train-suffix-r02.jsonl"
    ...
    for split, rows, expected_hash in (
        ("train", 2048, as_dict(launch["suffix"])["sha256"]),
