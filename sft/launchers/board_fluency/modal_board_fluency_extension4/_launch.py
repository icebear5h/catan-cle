...
MODAL_PROFILE=icebear5h CATAN_HF_SECRET_NAME=huggingface-secret-2     .venv/bin/python -B -m sft.launchers.board_fluency.modal_board_fluency_extension4     --run-name board-fluency-extension-20260915-r04 --budget-usd 41
...
from sft.launchers.board_fluency.modal_board_fluency_extension3._types import recorded_app_id
...
def launch(run_name: str, budget_usd: float = 41, execute: bool = False) -> JsonDict:
    ...
    return {**state, "local_receipt": str(local / "orchestration.json"),
            "stop": f"MODAL_PROFILE=icebear5h {sys.executable} -B -m sft.launchers.board_fluency.modal_board_fluency_extension4 --run-name {run_name} --stop"}
