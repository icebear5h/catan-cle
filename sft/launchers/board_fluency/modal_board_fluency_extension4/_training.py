...
from sft.launchers.board_fluency.modal_board_fluency_extension3._types import dig, json_list
...
from ._prepare import Extension4Callback, prepare_work, prepared_for, verify_initialization
...
def train_work(plan: JsonDict, deadline: float) -> JsonDict:
    ...
    return {"training": result, "final_checkpoint": str(final), "committed_steps": json_list(callback.saved_steps),
            ...
            "consumed": dig(plan, "suffix", "consumed"), "epoch3": dig(plan, "suffix", "epoch3"),
            "epoch4": dig(plan, "suffix", "epoch4"),
...
def bounded_stage(plan: JsonDict, stage: str, deadline: float) -> JsonDict:
    ...
    command = [sys.executable, "-B", "-m", "sft.launchers.board_fluency.modal_board_fluency_extension4", "--worker", stage,
