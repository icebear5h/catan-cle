...
from sft.scripts.eval.eval_qwen_vl_adapter import load_model, run_eval_job
...
class ModalGpuOptions(TypedDict):
    """`modal.App.function` GPU options, kept checkable through `**` unpacking."""
...
GPU_OPTIONS: ModalGpuOptions = dict(image=training_image, gpu="H200", cpu=(16.0, 16.0),
...
def preflight(payload: JsonDict) -> JsonDict:
    if not Path(SNAPSHOT).is_dir():
        raise FileNotFoundError("fresh base snapshot is not cached")
    tokenizer = load_tokenizer(SNAPSHOT)
    inventory = load_token_inventory(as_str(payload["token_inventory"]))
    tokenizer.add_tokens([AddedToken(as_str(t), normalized=False, special=False)
                          for t in as_list(inventory["atlas_tokens"])])
    train_jsonl = as_str(payload["train_jsonl"])
    report = inspect_jsonl_contract(train_jsonl, opt_str(payload["image_root"]), require_curriculum=False)
    lengths = [len(tokenizer.encode(at_str(r, "messages", -1, "content"), add_special_tokens=False))
               for _, r in iter_jsonl(Path(train_jsonl))]
...
def train(payload: JsonDict) -> JsonDict:
    os.environ["HF_HUB_OFFLINE"] = "1"
    torch.set_num_threads(16)
    config = train_config(payload)
    if (config.frozen_bundle or config.olora or config.resume_from_checkpoint
            or config.max_steps is None or not 1 <= config.max_steps <= 128):
    ...
    result: JsonDict = {"status": "running", "config": payload, "timeout_seconds": 3600}
...
@app.function(**GPU_OPTIONS, timeout=3600)
def evaluate(payload: JsonDict, steps: int, output_tag: str = "", all_layout_controls: bool = False) -> JsonDict:
    torch.set_num_threads(16)
    config = train_config(payload)
    if config.eval_jsonl is None:
        raise ValueError("evaluation requires the payload's eval_jsonl")
    ...
    started = time.monotonic()
    variants: JsonDict = {}
    result: JsonDict = {"status": "running", "checkpoint": str(checkpoint), "variants": variants}
    try:
        ...
        selected = [r for _, r in iter_jsonl(Path(eval_jsonl))]
        ...
        control_ids = [r["row_id"] for r in control_rows]
        result["control_ids"] = control_ids
        for label, source, variant in (("heldout", eval_jsonl, "original"),
...
def complete_pilot(payload: JsonDict, training_call_id: str) -> JsonDict:
    """Wait without a GPU, then run the fixed evaluation once training succeeds."""
    result: JsonDict = {"status": "waiting_for_training", "training_call_id": training_call_id}
    try:
        training = as_dict(modal.FunctionCall.from_id(training_call_id).get(timeout=3900))
        ...
        sft_runs.reload()
        write_json_atomic(Path(as_str(payload["output_dir"])) / "completed_pilot.json", result)
        sft_runs.commit()
    return result
...
def main(execute: bool = False, steps: int = 128, run_name: str = "full-board-fresh-20260907",
         eval_receipt: str = "", follow_receipt: str = "") -> None:
    if follow_receipt:
        receipt = as_dict(loads_json(Path(follow_receipt).read_text()))
        ...
        if not execute:
            ...
            return
        call = complete_pilot.spawn(as_dict(receipt["config"]), as_str(receipt["call_id"]))
        write_json_atomic(target, {"call_id": call.object_id, "training_call_id": receipt["call_id"],
                                  "config": receipt["config"]})
        print(target.read_text())
        return
    if eval_receipt:
        receipt = as_dict(loads_json(Path(eval_receipt).read_text()))
        payload = as_dict(receipt["config"])
        ...
        if not execute:
            ...
            return
        call = evaluate.spawn(payload, as_int(payload["max_steps"]))
    ...
    write_json_atomic(receipt_dir / "launch.json", {"call_id": call.object_id,
        ...
        "trainer_sha256": {p: sha256_file(Path(p)) for p in (
