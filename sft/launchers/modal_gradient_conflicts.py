"""Dry-run-first Catan behavior gradient diagnostics launcher in ``sft.launchers``."""
...
from sft.analysis.gradient_diagnostics import SelectedProbeRow
from sft.json_types import JsonLikeDict, as_str
from sft.launchers.gradient_conflicts import (
    JsonDict,
...
from sft.scripts.report.probe_gradient_conflicts import run_gradient_probe
...
def probe_h200(
    probe_args: JsonLikeDict,
    launch_payload: JsonLikeDict,
) -> JsonDict:
    arguments = probe_args_from_json(probe_args)
    output_dir = Path(arguments["output_dir"])
    manifest_path = output_dir / LAUNCH_MANIFEST
    identity = canonical_hash(launch_payload)
    existing = read_json_object(manifest_path) if manifest_path.is_file() else None
    ...
    write_json_atomic(manifest_path, launch)
    try:
        report = run_gradient_probe(**arguments)
...
def main(
    ...
    remote_dir = f"catan-gradient-probe/{as_str(selection['identity'])[:16]}"
    ...
    overview = {
        **launch,
        "identity": canonical_hash(launch),
        "dry_run": dry_run,
