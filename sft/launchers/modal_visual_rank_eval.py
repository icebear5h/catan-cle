...
from sft.analysis.visual_rank_eval import paired_summary, reconstruct_weight, select_terrain_boards
from sft.json_types import JsonLikeDict, as_dict, as_list, as_str, json_path, load_json_dict
from sft.launchers._hf import open_tensors
from sft.launchers._json import at, at_dict
...
from sft.scripts.report.extract_visual_delta import sha256_file, visual_keys, write_json
...
def evaluate(expected_content_sha256: str) -> JsonLikeDict:
    ...
    variants: list[JsonLikeDict] = []
    result: JsonLikeDict = {"schema": "catan_visual_rank_mini/v1", "status": "preflight", "variants": variants,
    ...
    try:
        rows = [as_dict(row) for _, row in iter_jsonl(Path(DATA) / "eval.jsonl")]
        ...
        write_json(output / "selection.json", {**manifest, "image_sha256": image_hashes})
        selected_path = output / "selected.jsonl"
        selected_path.write_text("".join(json.dumps(row) + "\n" for row in chosen))

        report = load_json_dict(Path(FACTORS) / "report.json")
        shards = [as_dict(shard) for shard in as_list(report["base_shards"])]
        tensors = [as_dict(row) for row in as_list(report["tensors"])]
        ...
        for shard in shards:
            if sha256_file(Path(SNAPSHOT) / as_str(shard["file"])) != shard["sha256"]:
                raise ValueError("base shard hash mismatch")
        index = at_dict(load_json_dict(Path(SNAPSHOT) / "model.safetensors.index.json"), "weight_map")
        base: dict[str, torch.Tensor] = {}
        for shard in shards:
            with open_tensors(str(Path(SNAPSHOT) / as_str(shard["file"])), framework="pt") as source:
                for row in tensors:
                    if index[as_str(row["base_key"])] == shard["file"]:
                        base[as_str(row["name"])] = source.get_tensor(as_str(row["base_key"])).bfloat16().float()
        trained_raw = load_file(Path(ADAPTER) / "visual_model.safetensors")
        trained = {as_str(row["name"]): trained_raw[as_str(row["trained_key"])] for row in tensors}
        factors: dict[str, dict[str, torch.Tensor]] = {}
        for row in tensors:
            if "factor_file" not in row:
                continue
            path = Path(FACTORS) / as_str(row["factor_file"])
            if sha256_file(path) != row["factor_sha256"]:
                raise ValueError(f"SVD factor hash mismatch: {path}")
            with open_tensors(str(path), framework="pt") as source:
                factors[as_str(row["name"])] = {key: source.get_tensor(key) for key in ("lora_A", "lora_B")}
        ...
        evidence["rank_sweep_precision"] = json_path(result, "precision")
        for label, rank in VARIANTS:
            ...
            records = [as_dict(row) for _, row in iter_jsonl(directory / "records.jsonl")]
            metrics = paired_summary(records, manifest["row_ids"])
            (directory / "strict_records.jsonl").write_text("".join(json.dumps(row) + "\n" for row in records))
            compact: JsonLikeDict = {
                       "label": label, "rank": rank, "rows": metrics["rows"], "heads": metrics["heads"],
                       "readouts": metrics["readouts"], "wrong_rows": metrics["wrong_rows"],
                       "failure_examples": metrics["failure_examples"],
                       "elapsed_seconds": time.monotonic() - variant_started,
                       "matrix_energy_captured": 1.0 if rank is None else (
                           0.0 if rank == 0 else at(report, "summary", "all", "matrix_energy_weighted_rank_capture", str(rank)))}
...
def main(dry_run: bool = True) -> None:
    _, manifest = select_terrain_boards([as_dict(row) for _, row in iter_jsonl(Path(LOCAL_VALIDATION))])
    if (manifest["source_rows"], manifest["rows"], manifest["layouts"]) != (3072, 240, 5):
        raise ValueError("unexpected local dataset coverage")
    plan: dict[str, object] = {"dry_run": dry_run, "output": OUTPUT, "gpu": "H200", "timeout_seconds": 900,
