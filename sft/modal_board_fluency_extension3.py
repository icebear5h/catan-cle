"""Opt-in 512-update r03 extension of r02; local dry runs never contact Modal.

MODAL_PROFILE=icebear5h CATAN_HF_SECRET_NAME=huggingface-secret-2 \
    .venv/bin/python -B -m sft.modal_board_fluency_extension3 \
    --run-name board-fluency-extension-20260915-r03 --budget-usd 31
...
def configuration(run_name: str, parent_config: dict) -> TrainConfig:
    ...
    if run_name in (PARENT_RUN, R06_RUN, R01_RUN, ext2.DEFAULT_RUN_NAME):
        raise ValueError("extension-r03 must have a fresh run name")
    if parent_config.get("max_steps") != 256 or parent_config.get("save_total_limit") != 8:
        raise ValueError("parent configuration is not the completed r02 256-step configuration")
    if (parent_config.get("save_steps") != 32 or parent_config.get("eval_steps") != 32
            ...
            or parent_config.get("initial_bundle") != R01_CHECKPOINT
            or parent_config.get("output_dir") != PARENT_ROOT + "/training"
            or parent_config.get("train_jsonl") != PARENT_ROOT + "/prepare/train-suffix-r02.jsonl"
            or parent_config.get("resume_from_checkpoint") is not None):
        raise ValueError("parent is not the exact completed r02 launch configuration")
    root = f"/runs/catan-vision-sft/{run_name}"
    config = replace(TrainConfig(**parent_config), initial_bundle=PARENT,
                     train_jsonl=root + "/prepare/train-suffix-r03.jsonl",
...
def budget_plan(budget_usd: float = 31) -> dict:
    if not math.isfinite(budget_usd) or not 0 < budget_usd <= 31:
        raise ValueError("budget must be positive and at most the approved $31 ceiling (user approved ~$31 for 512 repeat steps; never allow above 31)")
    ...
    return {"approved_usd": budget_usd, "ceiling_usd": 31,
            "user_approval": "user approved ~$31 for 512 repeat steps; never allow above 31",
            ...
            "prior_r01_cumulative_usd": float(PRIOR_R01_USD),
            "prior_r02_recorded_usd": float(R02_RECORDED_USD),
            "prior_recorded_window_plus_allowances_usd": float(PRIOR_USD),
            "prior_r01_cost_addendum_sha256": R01_ADDENDUM_SHA256,
            "prior_r01_total_key": R01_TOTAL_KEY,
            "prior_r02_cost_addendum_sha256": R02_ADDENDUM_SHA256,
            "prior_r02_subtotal_key": R02_SUBTOTAL_KEY,
...
def read_parent(root: Path) -> dict:
    ...
    if (launch["config"]["max_steps"] != 256 or launch["config"]["save_total_limit"] != 8
            or launch["config"]["save_steps"] != 32 or launch["config"]["eval_steps"] != 32
            or launch["config"]["initial_bundle"] != R01_CHECKPOINT
            or launch["config"]["output_dir"] != PARENT_ROOT + "/training"
            or launch["config"]["train_jsonl"] != PARENT_ROOT + "/prepare/train-suffix-r02.jsonl"
    ...
    for name, expected in launch["source_sha256"].items():
        if current.get(name) != expected:
            raise ValueError(f"a parent launcher/model/trainer/evaluator/scorer source changed since r02: {name}")
    ...
    state = shared.read_json(root / "training/checkpoints/checkpoint-256/trainer_state.json")
    if (state["global_step"] != 256
        ...
        raise ValueError("parent must be the completed, trained r02 checkpoint-256")
    for split, rows, expected_hash in (
        ("train", 2048, launch["suffix"]["sha256"]),
...
def suffix_identity(path: Path, inputs: dict) -> tuple[bytes, dict]:
    ...
    if set(epoch2["ids"]) != set(inputs["panels"]["train"]["ids"]):
    ...
    consumed_operations = Counter(epoch2["by_operation"]) + Counter(epoch3["by_operation"])
    consumed_families = Counter(epoch2["by_family"]) + Counter(epoch3["by_family"])
...
def build_plan(run_name: str, budget_usd: float = 31) -> dict:
...
def verify_plan(plan: dict) -> None:
    ...
    if plan["budget_claim"]["key"] in (ext1.COST_SHA256, ext2.CLAIM_KEY):
    ...
    if not math.isclose(float(r02["rates"]["gpu_stage_per_second_usd"]), plan["budget"]["gpu_stage_per_second"],
...
def check_schedule_log(state, logs: dict | None) -> None:
...
def check_schedule_history(trainer_state: dict) -> None:
...
def train_work(plan: dict, deadline: float) -> dict:
    ...
    return {"training": result, "final_checkpoint": str(final), "committed_steps": callback.saved_steps,
            ...
            "consumed": plan["suffix"]["consumed"], "epoch2": plan["suffix"]["epoch2"], "epoch3": plan["suffix"]["epoch3"],
...
def bounded_stage(plan: dict, stage: str, deadline: float) -> dict:
    ...
    command = [sys.executable, "-B", "-m", "sft.modal_board_fluency_extension3", "--worker", stage,
...
def coordinate_work(plan: dict, absolute_deadline: float, app_id: str) -> dict:
    ...
    try:
        ...
        } for panel in before}, additional_updates=512, cumulative_updates=1024)
    except BaseException as exc:
...
def launch(run_name: str, budget_usd: float = 31, execute: bool = False) -> dict:
    ...
    return {**state, "local_receipt": str(local / "orchestration.json"),
            "stop": f"MODAL_PROFILE=icebear5h {sys.executable} -B -m sft.modal_board_fluency_extension3 --run-name {run_name} --stop"}


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--run-name", default=DEFAULT_RUN_NAME)
    parser.add_argument("--budget-usd", type=float, default=31)
