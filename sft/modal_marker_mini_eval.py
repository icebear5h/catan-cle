...
from sft.marker_diagnostics import analyze, read_rows, select_rows, selection_manifest
...
def evaluate(expected_ids_hash: str):
    ...
    if manifest["rows"] != 308 or manifest["tokens"] != 154 or len(manifest["boards"]) != 5:
    ...
    for row in source:
        reference = row.get("image") or row["images"][0]
    ...
    command = [
        "python", "-m", "sft.scripts.eval_qwen_vl_adapter",
    ...
    try:
        ...
        try:
            ...
            run = {"status": status, "error": error, "elapsed_seconds": time.monotonic() - started,
...
def main(expected_ids_hash: str):
