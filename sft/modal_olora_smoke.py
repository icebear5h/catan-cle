...
from sft.modal_catan_vision_sft import (
    hf_cache, sft_data, sft_runs, training_image, upload_training_bundle,
)
from sft.scripts.train_trl_catan_vision import (
    MODEL_ID, PROFILE_OLORA_FROZEN_BUNDLE, TrainConfig, iter_jsonl,
    run_training, sha256_file, write_json_atomic,
)
...
def smoke(payload: dict) -> dict:
    ...
    result = {"status": "running", "config": payload, "timeout_seconds": 900}
    ...
    try:
        ...
        frozen = json.loads((output / "frozen_bundle.json").read_text())
        if frozen["unprotected_modules"]:
            raise RuntimeError("unprotected O-LoRA modules")
        if frozen["new_adapter"]["vision_targets"] != 110:
...
def main(execute: bool = False, prepare_only: bool = False, run_name: str = "olora-smoke-20260907"):
    ...
    for _, row in iter_jsonl(ROOT / "mixed_rung3b_v1/stage1/train.jsonl"):
        ...
        family = row.get("task", row.get("task_type", ""))
    ...
    write_json_atomic(receipt, {"call_id": call.object_id, "output": output,
                               ...
                               "source_sha256": {str(path): sha256_file(path) for path in (
                                   Path(__file__), Path("sft/scripts/train_trl_catan_vision.py"))}})
