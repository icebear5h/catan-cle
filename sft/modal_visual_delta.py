"""Bounded CPU-only extraction beside cached HF/checkpoint weights; never trains."""
...
def derive(hf_metadata: dict) -> dict:
    import torch
    from sft.scripts.extract_visual_delta import extract, write_json
    ...
    try:
        ...
        return {"output_dir": str(root / "derived"), "summary": report["summary"],
...
def main(hf_info: str, output: str, dry_run: bool = True):
    metadata = json.loads(Path(hf_info).read_text())
