...
Run: python -m sft.scripts.build_coordinate_comparison [--source-root PATH] [--output PATH]
...
from sft.coordinate_comparison import (
    DEFAULT_SOURCE, PAIR_QUOTAS, PROJECT_ROOT, REPRESENTATIONS, SCHEMA,
    build_comparison_rows, load_source_rows, mapping_artifact, mapping_sha256,
...
def prompt_length_report(rows: list[dict]) -> dict:
    result = {}
    for representation in REPRESENTATIONS:
        lengths = [len(r["messages"][0]["content"]) for r in rows
                   if r["metadata"]["representation"] == representation]
...
def render_preview(rows: list[dict]) -> str:
    ...
    for task in PAIR_QUOTAS:
        candidates = [r for r in rows if r["task_type"] == task
                      and r["metadata"]["representation"] == "atlas"]
        # Prefer a nonempty real answer for inspection, never alter selected cases.
        selected.append(next((r for r in candidates if r["metadata"]["answer"] != "NONE"), candidates[0]))
    lines = [
    ...
    by_pair = {r["metadata"]["pair_id"]: [] for r in rows}
    for row in rows:
        by_pair[row["metadata"]["pair_id"]].append(row)
    for chosen in selected:
        metadata = chosen["metadata"]
        lines.extend([f"## {metadata['area']} / {metadata['operation']}", "",
                      f"Source: `{metadata['source_id']}` ({metadata['source_split']}).", ""])
        for row in by_pair[metadata["pair_id"]]:
            prompt, answer = [m["content"] for m in row["messages"]]
            lines.extend([f"### {row['metadata']['representation']} ({len(prompt)} prompt characters)", "",
                          "```text", prompt, "```", "", "Gold:", "```text", answer, "```", ""])
    return "\n".join(lines)


def build_dataset(output_dir: Path = DEFAULT_OUTPUT, *, source_root: Path = DEFAULT_SOURCE) -> dict:
    ...
    manifest = {
        ...
        "code_sha256": {
            str(path.relative_to(PROJECT_ROOT)): file_sha256(path) for path in (
                Path(__file__).resolve(), PROJECT_ROOT / "sft/coordinate_comparison.py",
                PROJECT_ROOT / "sft/symbolic_board_tasks.py", PROJECT_ROOT / "sft/board_fluency_scoring.py",
                PROJECT_ROOT / "evals/catan_board_bench/tokens.py",
            )
        },
