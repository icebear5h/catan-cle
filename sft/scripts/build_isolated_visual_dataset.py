...
from catanbench.annotations import annotation_payload_for_contract  # noqa: E402
from catanbench.render import (  # noqa: E402
...
from catanbench.tokens import color_token, edge_token, node_token, resource_token, tile_token  # noqa: E402


DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "sft/data/isolated_visuals"
DEFAULT_CONTRACT = PROJECT_ROOT / "sft/fixtures/render_contracts/colonist_dummy_setup.json"
...
class DatasetWriter:
    ...
    def _message_row(self, row: dict[str, Any]) -> dict[str, Any]:
        image_path = (self.output_dir / row["image_path"]).resolve()
        return {
            "id": row["id"],
...
def add_tile_examples(writer: DatasetWriter, variant: int) -> None:
    for resource in RESOURCES:
        for number in NUMBERS:
            ...
            image, tile_bbox, number_bbox = render_tile_image(resource, number, writer.image_size, variant)
            writer.add_sample(
                sample_id=sample_id,
...
def add_robber_examples(writer: DatasetWriter, variant: int) -> None:
    for resource in [None, *RESOURCES]:
        ...
        for robber in [True, False]:
            ...
            image, tile_bbox, robber_bbox = render_robber_image(resource, robber, writer.image_size, variant)
            writer.add_sample(
                sample_id=sample_id,
...
def add_local_patch_examples(writer: DatasetWriter, contract_path: Path) -> None:
    ...
    full_image = render_contract_image(contract, image_size=full_image_size, style=DEFAULT_RENDER_STYLE)
...
def add_crop_sample(
    ...
    *,
    ...
    writer.add_sample(
        ...
        answer=answer,
        target=target,
        source={"kind": "local_board_crop", "full_image_size": full_image.size, "crop_size": crop_size},
        contract_path=contract_path,
    )
...
def render_tile_image(
    ...
    tile_bbox = paste_asset(canvas, _asset_path(f"/assets/tiles/{asset_name}.svg"), tile_x, tile_y, tile_size, tile_size)
    number_bbox = None
...
def render_road_image(color: str | None, image_size: int, variant: int) -> tuple[Image.Image, list[int] | None, int]:
    canvas = base_canvas(image_size)
...
def render_node_image(
    ...
    canvas = base_canvas(image_size)
    draw = ImageDraw.Draw(canvas, "RGBA")
    center = (image_size // 2 + variant_offset(variant)[0], image_size // 2 + variant_offset(variant)[1])
...
def render_port_image(resource: str | None, image_size: int, variant: int) -> tuple[Image.Image, list[int]]:
    canvas = base_canvas(image_size)
    slug = "generic" if resource is None else resource.lower()
...
def render_robber_image(
    ...
    canvas, tile_bbox, _ = render_tile_image(resource, None if resource is None else 8, image_size, variant)
    ...
    if robber:
        ...
        robber_bbox = paste_asset(canvas, _asset_path("/assets/pieces/robber.svg"), x, y, size, size)
...
def paste_asset(canvas: Image.Image, path: Path, x: int, y: int, width: int, height: int) -> list[int]:
    asset = _raster_asset(path, width, height)
    canvas.alpha_composite(asset, (x, y))
    return [x, y, x + width, y + height]


def crop_square(image: Image.Image, center_x: int, center_y: int, crop_size: int, output_size: int) -> Image.Image:
    half = crop_size // 2
    left = center_x - half
    top = center_y - half
    ...
    crop.paste(image.crop((source_left, source_top, source_right, source_bottom)), (paste_x, paste_y))
