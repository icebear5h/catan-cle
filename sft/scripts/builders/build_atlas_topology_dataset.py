...
from sft.json_types import JsonLikeDict, as_dict, as_str

DEFAULT_PROMPT_PREFIX = "Answer exactly using Catan atlas tokens. Do not explain."


def message_row(row_id: str, category: str, question: str, answer: str,
                target: JsonLikeDict) -> JsonLikeDict:
    return {
        "id": row_id,
        "messages": [
            {
...
def tokens(items: Sequence[Mapping[str, object]], key: str = "token") -> list[str]:
    return [str(item[key]) for item in items]


class TopologyIndices(TypedDict):
    tiles: dict[int, AtlasTile]
    nodes: dict[int, AtlasNode]
    edges: dict[tuple[int, int], AtlasEdge]
    ports: dict[int, AtlasPort]
...
def _pair(edge: list[int]) -> tuple[int, int]:
    a, b = edge
    return a, b


def build_indices(atlas: AtlasMetadata) -> TopologyIndices:
    tiles = {tile["id"]: tile for tile in atlas["tiles"]}
    nodes = {node["id"]: node for node in atlas["nodes"]}
    edges = {_pair(edge["id"]): edge for edge in atlas["edges"]}
...
def main() -> int:
    ...
    categories = sorted({as_str(as_dict(row["metadata"])["category"]) for row in rows})
