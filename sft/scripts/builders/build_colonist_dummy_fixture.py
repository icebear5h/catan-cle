...
def build_fixture() -> JsonLikeDict:
    ...
    for tile_id in sorted(tile_refs):
        ...
        tile: JsonLikeDict = {
            ...
            "edges": [
                list(edge)
                for edge in sorted(
                    canonical_edge(_pair(edge)) for edge in tile_ref["edges"].values()
                )
            ],
            "edge_tokens": [
                edge_token(edge)
    ...
    edges: list[JsonLikeDict] = []
    for edge_ref in atlas["edges"]:
        edge = _pair(edge_ref["id"])
