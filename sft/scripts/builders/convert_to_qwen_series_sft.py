...
from sft.json_types import JsonDict, JsonValue, as_dict, as_list, as_str
...
def _assistant_text(message: JsonDict) -> str:
    content = message.get("content", "")
    if isinstance(content, list):
        parts = [as_dict(entry) for entry in content]
        return "\n".join(
...
def convert_file(
    ...
    *,
    ...
    for _, entry in iter_jsonl(input_path):
        row = as_dict(entry)
        image_name = None
        if row.get("image"):
            image_path = resolve_dataset_asset(input_path, as_str(row["image"]))
            if copy_images_to is not None:
