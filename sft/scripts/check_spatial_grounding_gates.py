...
def _dimension_accuracy(summary: JsonDict, dimension: str, value: str) -> float:
    bucket = summary.get(f"by_{dimension}", {}).get(value)
    if not bucket:
        raise ValueError(f"summary has no {dimension}={value!r} bucket")
    return float(bucket["exact_accuracy"])


def _gate(name: str, value: float, threshold: float, operator: str) -> JsonDict:
    if operator == ">=":
...
def _patch_eval_accuracy(metrics: JsonDict) -> float:
    eval_metrics = metrics.get("eval") or metrics
...
def stage1_gates(
    *,
    ...
    treatment_probe = float(probe["exact_accuracy"])
    strongest_control = max(
        float(marker_only_probe["exact_accuracy"]),
        float(shuffled_target_probe["exact_accuracy"]),
    )
    occlusion_gap = float(control_occlusion_probe["exact_accuracy"]) - float(
        target_occlusion_probe["exact_accuracy"]
    )
    gates = [
        _gate("marker_overall_exact", float(marker["exact_accuracy"]), 0.97, ">="),
...
def _minimum_bucket_accuracy(summary: JsonDict, dimensions: Iterable[str]) -> tuple[float, str]:
    ...
    for dimension in dimensions:
        for name, bucket in summary.get(f"by_{dimension}", {}).items():
            if name != "unknown":
                values.append((float(bucket["exact_accuracy"]), f"{dimension}={name}"))
    if not values:
...
def stage2_gates(
    *,
    ...
    marker_regression = float(stage1_marker["exact_accuracy"]) - float(
        stage2_marker["exact_accuracy"]
    )
    gates = [
        _gate("orientation_overall_exact", float(orientation["exact_accuracy"]), 0.95, ">="),
