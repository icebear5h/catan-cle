...
def load_render_style(path: Path | None) -> RenderStyle | None:
    ...
    style_payload = payload.get("style", payload)
    style_keys = RenderStyle.__dataclass_fields__.keys()
    return RenderStyle(**{key: style_payload[key] for key in style_keys if key in style_payload})
...
def main() -> int:
    ...
    with image_manifest_path.open("w") as manifest_out:
        for index, (_, row) in enumerate(iter_jsonl(manifest_path)):
            if args.limit_samples is not None and index >= args.limit_samples:
                break
            sample_id = row["sample_id"]
            contract_rel = Path(row["contract_path"])
            ...
            row_with_image = dict(row)
            row_with_image["image_path"] = str(image_rel)
            if isinstance(row_with_image.get("source"), dict):
                row_with_image["source"] = dict(row_with_image["source"])
                row_with_image["source"]["render_status"] = "rendered_python_hexboard"
    ...
    with (
        ...
        for _, qa in iter_jsonl(qa_path):
            image_path = rendered.get(qa["sample_id"])
