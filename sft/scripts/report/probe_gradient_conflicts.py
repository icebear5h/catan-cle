...
from sft.json_types import as_dict, as_int, as_list, as_str, json_path, load_json_dict
...
def probe_config_from_bundle(
    ...
    *,
    ...
    parent = load_json_dict(parent_path)
    valid_fields = {field.name for field in fields(TrainConfig)}
    payload: JsonDict = {name: value for name, value in parent.items() if name in valid_fields}
    payload.update(
...
def _validate_selection_rows(rows: list[JsonDict], selection: JsonDict) -> None:
    if len(rows) != as_int(selection["total_rows"]):
        raise ValueError("probe JSONL row count disagrees with selection manifest")
    for behavior in PROBE_BEHAVIORS:
        spec = as_dict(json_path(selection, "behaviors", behavior))
        observed = [
            str(rows[index].get("row_id") or rows[index].get("id"))
            for index in (as_int(value) for value in as_list(spec["indices"]))
        ]
...
def run_gradient_probe(
    *,
    ...
    selection = load_json_dict(selection_path)
    output = Path(output_dir)
    report_path = output / "gradient_conflicts.json"
    if report_path.is_file():
        existing = load_json_dict(report_path)
        if (
            Path(as_str(existing["adapter_dir"])).resolve() != Path(adapter_dir).resolve()
            or json_path(existing, "selection", "identity") != selection["identity"]
        ):
    ...
    for behavior in PROBE_BEHAVIORS:
        count = len(as_list(json_path(selection, "behaviors", behavior, "indices")))
    ...
    for behavior in PROBE_BEHAVIORS:
        spec = as_dict(json_path(selection, "behaviors", behavior))
        indices = [as_int(value) for value in as_list(spec["indices"])]
        row_ids = [as_str(value) for value in as_list(spec["row_ids"])]
        ...
        for start in range(0, len(indices), microbatch_size):
            ...
            if not isinstance(loss, torch.Tensor):
                raise TypeError("compute_loss without return_outputs must return a tensor")
