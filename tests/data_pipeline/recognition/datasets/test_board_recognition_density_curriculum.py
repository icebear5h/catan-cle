...
from sft.board.density_curriculum import (
...
def record(
    index: int, *, density: str, pieces: int, head: str, class_name: str
...
def test_curriculum_orders_stages_and_preserves_each_row_once() -> None:
    pairs = [
    ...
    annotations: Any = [pair[0] for pair in pairs]
    audits: Any = [pair[1] for pair in pairs]
...
def test_curriculum_rejects_piece_count_outside_density_boundary() -> None:
...
def test_sequential_sampler_cannot_shuffle_curriculum_rows() -> None:
    data = list(range(8))
...
def test_manifest_stage_lookup_and_strict_row_count(tmp_path: Path) -> None:
    manifest = {
