...
def write_ledger(path: Path, game_ids: list[str]) -> None:
    path.write_text(
        json.dumps(
            {
...
def test_audit_replay_accepts_complete_issue_free_source(tmp_path: Path) -> None:
...
def test_audit_replay_allows_explicit_nonvisual_trade_diagnostic(tmp_path: Path) -> None:
...
def test_audit_replay_rejects_board_unsafe_semantic_issue(tmp_path: Path) -> None:
    replay: Any = tmp_path / "101.json"
    replay.write_text("{}")
...
def test_source_lock_excludes_benchmark_before_replay_loading(tmp_path: Path) -> None:
    ...
    def loader(path: str | Path, **_kwargs: object) -> SimpleNamespace:
        loaded.append(Path(path).stem)
        return fake_state()

    lock: Any = build_replay_source_lock(
...
def test_source_lock_fails_closed_below_minimum(tmp_path: Path) -> None:
    replay_dir = tmp_path / "replays"
    replay_dir.mkdir()
...
def test_source_lock_path_migration_preserves_historical_metadata_identity(tmp_path: Path) -> None:
    replay_dir: Any = tmp_path / "replays"
    replay_dir.mkdir()
    (replay_dir / "200.json").write_text("{}")
    ledger: Any = tmp_path / "benchmark.json"
    write_ledger(ledger, ["100"])
    ...
    variants: Any = source_lock_identity_variants(lock)
    historical: Any = next(
...
def test_source_lock_hash_detects_tampering(tmp_path: Path) -> None:
    replay_dir = tmp_path / "replays"
    replay_dir.mkdir()
    (replay_dir / "200.json").write_text("{}")
    ledger = tmp_path / "benchmark.json"
