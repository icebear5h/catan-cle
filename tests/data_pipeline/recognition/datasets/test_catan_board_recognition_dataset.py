...
def test_state_dataset_returns_balanced_queries_and_raw_image() -> None:
...
def test_state_query_sampling_is_deterministic_and_epoch_variant() -> None:
    dataset = BoardRecognitionStateDataset(
        FIXTURE_DIR,
        split="validation",
        queries_per_state=16,
    ...
    dataset.set_epoch(1)
    epoch_one: Any = dataset[0]["queries"]
...
def test_counterfactual_pair_samples_aligned_queries() -> None:
    dataset = BoardRecognitionStateDataset(
        FIXTURE_DIR,
        split="train",
        queries_per_state=16,
    ...
    target: Any = first["counterfactual_target"]
...
def test_state_dataloader_collates_one_image_with_many_queries() -> None:
    dataset = BoardRecognitionStateDataset(
        FIXTURE_DIR,
        split="train",
        queries_per_state=16,
...
def test_worker_processes_receive_epoch_updates() -> None:
    dataset = BoardRecognitionStateDataset(
        FIXTURE_DIR,
        split="train",
        queries_per_state=16,
...
def test_qwen_sft_export_has_official_conversation_shape(tmp_path: Path) -> None:
    ...
    rows: Any = read_jsonl(output_dir / "train.jsonl")
    audit: Any = read_jsonl(output_dir / "audit/train.jsonl")
...
def test_qwen_sft_export_is_deterministic_except_timestamp(tmp_path: Path) -> None:
