...
from data_pipeline.board_recognition.mix_rung_data import (
    apportion,
    draw_cells,
    estimate_tokens,
    export_mixed_rung,
...
def test_apportion_and_token_estimate() -> None:
...
def test_mixed_rung_draws_quotas_by_density_and_balance(tmp_path: Path) -> None:
...
def test_draw_cells_oversamples_short_cells_up_to_max_repeats() -> None:
    pool: Any = [{"row_id": f"r{i}", "color": "WHITE" if i < 2 else "RED"} for i in range(6)]
    picked, remainder = draw_cells(pool, {("WHITE",): 5, ("RED",): 2}, ["color"], "salt", max_repeats=3)
    whites: Any = [row for row in picked if row["color"] == "WHITE"]
