...
from sft.scripts.train.train_trl_catan_vision import inspect_jsonl_contract

JsonRow = dict[str, Any]


@pytest.fixture
def source_rows() -> list[JsonRow]:
    ...
    for index, colour in enumerate(ALL_COLORS):
        ...
        rows = rows_for_state(
            dict(state, sample_id=f"train_{index}", split="train"), board, coverage="capped"
        )
        result.extend(rows)
    return result
...
def test_pool_expands_all_existing_labels_without_mutating_inputs(source_rows: list[JsonRow]) -> None:
    before = copy.deepcopy(source_rows)
    pool = training_pool(source_rows)
    by_board: Any = {(r["state_id"], r["task_type"].removesuffix("_readout")): parse_readout(r)
                for r in source_rows if group(r) == "readout"}
    positives: Any = [r for r in pool if group(r) == "occupied"]
...
def test_pool_rejects_bad_inputs(source_rows: list[JsonRow], fault: str) -> None:
...
def test_token_budget_balances_colour_piece_and_preserves_full_answers(source_rows: list[JsonRow]) -> None:
    pool: Any = training_pool(source_rows)
    ...
    result: Any = audit(sampled, count_tokens)
    ...
    for piece in ("ROAD", "CITY", "SETTLEMENT"):
        masses: Any = [result["short_positive_colour_piece_tokens"][f"{c}/{piece}"] for c in ALL_COLORS]
...
def test_invalid_config_fails(config: MixConfig) -> None:
    with pytest.raises(ValueError):
        config.validate()


def test_missing_buckets_and_repeat_cap_fail_closed(source_rows: list[JsonRow]) -> None:
    pool = training_pool(source_rows)
...
def test_apportion_is_exact_and_stable() -> None:
    assert apportion({"b": 1.0, "a": 1.0, "c": 1.0}, 5) == {"b": 2, "a": 2, "c": 1}


def _export_fixture(tmp_path: Path, source_rows: list[JsonRow]) -> tuple[Path, Path]:
...
def test_export_preserves_evaluation_and_source_bytes_and_image_contract(tmp_path: Path, source_rows: list[JsonRow]) -> None:
    ...
    result: Any = export_reweighted(source, output, tokenizer, row_count=30000, config=MixConfig(max_repeats=1000))
...
def test_export_checks_fingerprints_before_writing(tmp_path: Path, source_rows: list[JsonRow]) -> None:
    source, tokenizer = _export_fixture(tmp_path, source_rows)
