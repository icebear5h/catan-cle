...
def test_compact_node_signatures_include_codes_words_and_uniqueness() -> None:
    signatures: Any = compact_node_signatures(contract())
...
def test_inverse_descriptions_do_not_leak_atlas_tokens_or_keep_collisions() -> None:
...
def test_inverse_state_has_four_token_targets_and_piece_qualified_rows() -> None:
    queries: Any = inverse_queries_for_state(state(), contract(), state_index=73)
...
def test_inverse_rows_and_audits_pass_schemas() -> None:
...
def test_inverse_row_rejects_token_leakage_and_multi_token_answers() -> None:
    query = inverse_queries_for_state(state(), contract(), state_index=73)[1]
    row: Any = inverse_ms_swift_row(query, image_name="fixture_state.png")
...
def test_inverse_contract_records_alias_and_mix_policy() -> None:
    payload: Any = inverse_grounding_contract()
