...
def test_split_scheduler_is_atomic_balanced_complete_and_deterministic(tmp_path: Path) -> None:
    ...
    metrics: Any = validate_split_balance(first, split="train")
...
def test_epoch_identity_changes_the_plan_without_changing_gates(tmp_path: Path) -> None:
    states = make_schedule_fixture(tmp_path)
...
def test_state_dataset_loads_one_image_with_eight_scheduled_queries(tmp_path: Path) -> None:
...
def test_qwen_projection_has_one_image_two_query_tokens_and_one_answer_token(tmp_path: Path) -> None:
    states = make_schedule_fixture(tmp_path)
    plan: Any = build_split_query_plan(
        states,
        dataset_dir=tmp_path,
        split="train",
        seed=77,
    ...
    row: Any = qwen_row(query, image_name="board.png")
