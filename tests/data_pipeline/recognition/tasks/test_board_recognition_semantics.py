...
def test_semantic_projection_trains_154_bidirectional_location_rows() -> None:
...
def test_historical_220_token_inventory_is_unchanged() -> None:
...
def test_semantic_prompts_use_one_location_token_and_ordinary_language(head: str, slot: str, expected: str) -> None:
...
def test_semantic_answers_are_controlled_phrases(head: str, class_name: str, expected: str) -> None:
    assert semantic_answer(head, class_name) == expected


def test_every_engine_class_has_one_unique_legal_answer() -> None:
    contract: Any = semantic_contract()

    assert set(contract["heads"]) == set(QUERY_TEXT_BY_HEAD)
    for head, classes in RECOGNITION_CLASS_VOCABULARIES.items():
        candidates: Any = semantic_candidates(head)
...
def test_semantic_prompt_and_answer_reject_cross_head_or_unknown_values() -> None:
