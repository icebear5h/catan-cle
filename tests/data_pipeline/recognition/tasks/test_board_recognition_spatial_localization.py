...
def test_renderer_aligned_regions_cover_the_full_atlas() -> None:
    _, contract = _fixture_contract()

    regions: Any = atlas_regions(contract, image_size=1024, view_padding_factor=1.2)
...
def test_marker_groups_are_local_same_type_and_never_singletons() -> None:
    _, contract = _fixture_contract()
...
def test_marker_rows_are_bidirectional_and_have_control_bboxes(tmp_path: Path) -> None:
    ...
    rows: Any = marker_rows_for_board(
        state=state,
        contract=contract,
        base_image=image,
...
def test_entity_markers_draw_edge_bars_along_the_edge(tmp_path: Path) -> None:
...
def test_relation_stage_expands_every_canonical_fact_eight_times() -> None:
    states: Any = [
...
def test_relation_stage_preserves_balanced_choices_and_rendered_directions(
    repetitions: int, balance_polarity: bool
) -> None:
    state, contract = _fixture_contract()
    regions: Any = atlas_regions(contract, image_size=1024, view_padding_factor=1.2)
    rows: Any = _relation_rows(
        [{**state, "split": "train"}],
    ...
    for row in _deterministic_shuffle(rows, "stage2"):
        ...
        choices: Any = prompt.rsplit(": ", 1)[1].removesuffix("?").split(" or ")
        answer: Any = row["messages"][1]["content"]
        assert choices == row["tokens"]
        position: Any = choices.index(answer)
        counts[(row["entity_type"], position)] += 1
        other: Any = choices[1 - position]
...
def test_balanced_relation_stage_equalizes_yes_and_no_per_relationship() -> None:
    states: Any = [
        {"sample_id": f"empty_{index}", "split": "train"}
        for index in range(8)
    ]
    ...
    balanced: Any = _relation_rows(states, repetitions=8, balance_polarity=True)
    ...
    for row in balanced:
        if row["polarity"] in {"positive", "hard_negative"}:
            key: Any = (row["entity_type"], row["relationship"], row["polarity"])
...
def test_deterministic_shuffle_is_a_stable_permutation() -> None:
    rows: Any = [{"row_id": f"row_{index:03d}", "sampling_repeat": index % 2} for index in range(64)]

    first: Any = _deterministic_shuffle(rows, "stage1")
...
def test_neutral_probe_covers_every_node_and_edge_once(tmp_path: Path) -> None:
    fixture_state, contract = _fixture_contract()
    state = {
        **fixture_state,
        "split": "test",
    ...
    small: Any = [row for row in rows if row["probe_style"] == "heldout_gray_dot_small"]
    large: Any = [row for row in rows if row["probe_style"] == "heldout_gray_dot_large"]
