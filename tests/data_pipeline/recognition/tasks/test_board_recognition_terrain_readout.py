...
def _fixture_state(
    sample_id: str = "empty_setup_node_p000_base",
...
def test_layout_and_port_answers() -> None:
...
def test_rows_cover_every_tile_and_port_with_desert_and_duplicates() -> None:
    ...
    prompts: Any = {row["messages"][0]["content"].replace("<image>\n", ""): row["messages"][1]["content"] for row in rows}
    ...
    readout: Any = next(row for row in rows if row["task_type"] == "terrain_readout")
...
def test_readout_answer_keeps_duplicate_tiles_distinct() -> None:
    tiles: Any = [{"token": "<T00>", "resource": "wheat", "number": "8"}, {"token": "<T01>", "resource": "wheat", "number": "8"}]
    ports: Any = [{"token": "<P00>", "answer": "3:1 port"}]
...
def test_export_splits_by_layout() -> None:
    output = FIXTURE_ROOT / "terrain_readout_test_output"
    _remove_tree(output)
    try:
        metadata: Any = export_terrain_readout(FIXTURE_ROOT, output_dir=output, overwrite=True, validate_dataset=False)
...
def test_synthetic_boards_are_seeded_distinct_layouts() -> None:
    ...
    rows: Any = rows_for_state(state_a, contract_a)
