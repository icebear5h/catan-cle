...
def test_longest_road_counts_trails_not_vertices_or_total_pieces(
...
def test_enemy_endpoints_count_terminal_edges_without_joining_roads(piece: str) -> None:
    game = _position(
...
def test_blocked_loop_can_end_but_cannot_pass_through_enemy(blockers: tuple[int, ...], length: int, components: int) -> None:
    game = _position(
...
def test_own_settlement_does_not_cut_road() -> None:
    game = _position(((Color.RED, (0, 1, 2, 3, 4, 5, 0)),), ((Color.RED, 0),))
    assert game.state.board.road_lengths[Color.RED] == 6


def test_recomputed_three_road_junction_splits_all_arms() -> None:
    game = _position(
...
def test_road_may_approach_enemy_but_cannot_extend_from_it(reverse: bool) -> None:
...
def test_enemy_endpoint_is_legal_when_other_endpoint_independently_connects() -> None:
    game = _position(
...
def test_five_road_ties_keep_only_an_eligible_incumbent(incumbent: Color | None) -> None:
    game = _position(
...
def test_cut_revokes_award_when_two_challengers_tie_then_unique_leader_acquires() -> None:
    game = _position(
        ((Color.RED, (29, 30, 31, 32, 33, 34)),
...
def test_award_revocation_below_five_and_reacquisition_preserve_hidden_vp() -> None:
    game = _position(
...
def test_recompute_replaces_stale_replay_components_lengths_and_edge_cache() -> None:
