...
def _advance_initial_placement(game: GameEngine) -> None:
    while game.state.is_initial_build_phase:
        game.step(game.state.playable_actions[0])


def _roll_action(game: GameEngine) -> Action:
    return next(
        action
...
def _board_fingerprint(game: GameEngine) -> tuple[Any, ...]:
    return tuple(
        sorted(
            (
                coordinate,
...
def test_game_does_not_mutate_process_global_random_state() -> None:
    random.seed(90210)
...
def test_same_seed_games_match_when_another_game_is_interleaved() -> None:
    first = _game(77)
    unrelated = _game(991)
    second = _game(77)
...
def test_game_copy_has_an_independent_identical_rng_continuation() -> None:
    game = _game(1234)
    _advance_initial_placement(game)
    branch = game.copy()
...
def test_undo_restores_rng_so_repeating_action_repeats_outcome() -> None:
    game = _game(2026)
    _advance_initial_placement(game)
