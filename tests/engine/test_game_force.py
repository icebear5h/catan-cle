...
def test_forced_actions_reject_random_or_missing_values(action: Action) -> None:
    with pytest.raises(ValueError, match="Forced"):
        make_game().step(action, force=True)


def test_apply_action_force_rejects_random_placeholders_directly() -> None:
    game = make_game()
...
def test_unforced_random_roll_returns_and_logs_resolved_action() -> None:
    game = make_game()
...
def test_forced_explicit_roll_is_accepted_without_randomizing() -> None:
    game = make_game()
