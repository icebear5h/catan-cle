...
from scripts.board_bench.run import eval_catan_board_bench_full_graph_formats as base_evaluator
from scripts.board_bench.run.eval_catan_strict_text_probe_novita import (
    DEFAULT_DATASET_DIR,
    FORMAT_NAME,
    admissible_record,
    build_plan,
...
def test_text_runner_uses_locked_winner_and_strict_scoring(tmp_path: Path) -> None:
    with configured_text_evaluator():
        metadata: Any = json.loads((DEFAULT_DATASET_DIR / "metadata.json").read_text())
        base_evaluator.validate_dataset_metadata(metadata)
        questions: Any = base_evaluator.validate_dataset_integrity(DEFAULT_DATASET_DIR, metadata)
        jobs: Any = base_evaluator.build_jobs(
            DEFAULT_DATASET_DIR,
            questions=questions,
            formats=[FORMAT_NAME],
            max_requests=None,
    ...
    plan: Any = build_plan(args, metadata=metadata, questions=questions, jobs=jobs)
    job: Any = jobs[0]
    result = {
...
def test_novita_text_call_disables_thinking(monkeypatch: pytest.MonkeyPatch) -> None:
    captured: Any = {}
    ...
    class FakeResponse:
        ...
        def json(self) -> dict[str, str | list[dict[str, dict[str, str]]] | dict[str, int]]:
            return {
                "model": "qwen/qwen3.8-max",
    ...
    monkeypatch.setattr(
        "scripts.board_bench.run.eval_catan_strict_text_probe_novita.httpx.Client",
        FakeClient,
    )
