...
def slow_request_worker(
    _result_sender: object, _api_key: str, _args: object, _job: object
) -> None:
    time.sleep(0.2)


def test_question_projection_inverts_text_aliases_to_engine_ids() -> None:
...
def test_rendered_probe_uses_raw_images_and_strict_scoring(tmp_path: Path) -> None:
    source_metadata, source_manifest, source_questions = validate_source_dataset(DEFAULT_SOURCE_DIR)
    output_dir: Any = tmp_path / "strict_vision_probe"
    ...
    jobs: Any = build_jobs(output_dir, manifest=manifest, questions=questions)
    ...
    openrouter_args: Any = SimpleNamespace(**vars(args), provider="openrouter")
    openrouter_plan: Any = build_plan(
        openrouter_args,
        metadata=metadata,
        questions=questions,
        jobs=jobs,
    )
    openrouter_result: Any = dict(result, provider="AkashML")
...
def test_sequential_request_has_a_hard_wall_clock_deadline() -> None:
    args: Any = SimpleNamespace(timeout=0.02)

    result: Any = call_job_with_hard_deadline(
        "key",
        args,
        {"qa": {"id": "q"}},
