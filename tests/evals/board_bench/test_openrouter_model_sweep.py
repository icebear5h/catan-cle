...
from scripts.board_bench.run.run_catan_openrouter_model_sweep import (
    DEFAULT_CONFIG,
    board_command,
    model_key,
...
def fake_catalog(config: dict[str, Any]) -> dict[str, Any]:
    return {
        "data": [
            {
                "id": row["model_id"],
...
def test_sweep_config_is_unique_multimodal_and_within_budget(tmp_path: Path) -> None:
    config: Any = load_config()
    selected: Any = validate_config(config, fake_catalog(config))
...
def test_sweep_rejects_catalog_candidate_without_image_input() -> None:
    config = load_config()
    catalog: Any = fake_catalog(config)
...
def test_sweep_commands_lock_board_and_direct_reasoning_conditions(tmp_path: Path) -> None:
    config: Any = load_config()
    model_id: Any = config["candidates"][0]["model_id"]
    board = board_command(config, tmp_path, model_id, full=False)
    placement_models: Any = [
        row["model_id"]
        for row in config["candidates"]
    ...
    assert reasoning[1:3] == ["-m", "scripts.reasoning.eval_catan_initial_settlement_reasoning"]
    ...
    strategy_models: Any = [model for model in placement_models if model not in excluded]
...
def test_sweep_cost_parser_and_model_key_are_stable() -> None:
