...
def _features(
    *,
    ...
    actor_actual: int = 2,
    opponent_max: int = 2,
    max_public: int | None = None,
    actions: tuple[str, ...] = (),
    road_length: int = 0,
    played_knights: int = 0,
    turn_owner: bool = True,
) -> dict[str, Any]:
    return {
...
def test_bucket_suite_is_strict_versioned_and_complete() -> None:
...
def test_bucket_suite_rejects_unknown_fields(tmp_path: Path) -> None:
...
def test_setup_ordinals_produce_first_second_and_shared_road_buckets() -> None:
    records = [
...
def test_early_robber_and_trade_decisions_are_multi_labelled() -> None:
    records = [
        _record(
            20,
            "MOVE_ROBBER",
...
def test_late_critical_turn_combines_win_denial_and_end_turn_buckets() -> None:
    record = _record(
        90,
        "END_TURN",
        features=_features(
...
def test_state_feature_capture_uses_public_opponent_vp_and_actor_private_vp() -> None:
    game = GameEngine(
