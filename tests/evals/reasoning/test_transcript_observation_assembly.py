...
def observation_scan() -> dict[str, Any]:
    return scan_observation_jobs("242781000")


def test_global_reflow_assigns_cross_boundary_utterance_when_it_finishes() -> None:
    transcript = {
        "segments": [
            {
                "source_index": 0,
...
def test_packet_anchors_insert_bounded_observation_before_next_decision() -> None:
    evidence = [
        {
...
def test_reordered_same_event_decision_uses_first_safe_viewer_cursor() -> None:
...
def test_full_scan_builds_decision_and_observation_packets_without_drops(
    observation_scan: dict[str, Any],
) -> None:
    jobs: Any = observation_scan["jobs"]
    evidence_ids = [
...
def test_assembly_parser_derives_strict_causal_provenance(observation_scan: dict[str, Any]) -> None:
...
def test_assembly_parser_requires_exact_evidence_partition(observation_scan: dict[str, Any]) -> None:
    job = observation_scan["jobs"][0]
...
def test_assembly_parser_rejects_decision_reasoning_at_observation(
    ...
    job: Any = next(job for job in observation_scan["jobs"] if job.anchor_kind == "observation")
    evidence_ids = [item["evidence_id"] for item in job.utterances]
