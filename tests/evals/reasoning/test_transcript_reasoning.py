...
@pytest.fixture
def opening_reasoning_job() -> Iterator[ReasoningJob]:
...
def test_reasoning_job_exposes_public_board_and_grounded_caption_locations(
    opening_reasoning_job: ReasoningJob,
) -> None:
...
def test_reasoning_parser_rejects_unknown_evidence(opening_reasoning_job: ReasoningJob) -> None:
    raw_response = json.dumps(
        {
            "paragraphs": [
                {
...
def test_reasoning_generation_preserves_failed_response_provenance(
    ...
    def fake_query(*args: object, **kwargs: object) -> dict[str, ]:
        return {
