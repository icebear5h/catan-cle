...
def test_public_board_snapshot_is_private_state_independent_and_immutable() -> None:
    engine = _engine()
...
def test_indexed_tile_rows_surface_is_complete_canonical_and_parseable() -> None:
...
def test_image_surface_is_bounded_unannotated_and_state_paired() -> None:
    engine = _engine()
    ...
    metadata: Any = board_presentation_payload(image, include_text_content=True)
    assert metadata["data"] is None
    assert image.data.hex() not in str(metadata)


def test_board_presentation_contract_rejects_tampering() -> None:
    context = build_decision_context(_engine())
    text = IndexedTileRowsBoardPresenter().present(context)
    image: Any = ImageBoardPresenter(image_size=512).present(context)
...
def test_openai_board_encoding_attaches_only_to_latest_user_turn() -> None:
    messages = (
    ...
    text_payload: Any = openai_messages_with_board(
        messages,
        text,
    ...
    content: Any = image_payload[-1]["content"]
