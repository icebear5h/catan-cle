...
def test_exactly_one_active_suite_and_it_is_the_default() -> None:
...
def test_legacy_pair_defaults_are_marked_legacy_not_deprecated() -> None:
    statuses = _status_by_file()
    assert statuses["catan_v11.yaml"] == "legacy"
    assert statuses["communication_v5.yaml"] == "legacy"


def test_deprecated_files_carry_a_banner_comment() -> None:
...
def test_loading_deprecated_suite_warns_and_active_does_not() -> None:
