"""Request payloads, reasoning flags, and retries."""
import json
...
@pytest.mark.asyncio
async def test_openrouter_transport_sends_complete_context_and_session_affinity() -> None:
    captured = {}
    ...
    response: Any = await transport.complete(model_request)
    await client.aclose()
...
@pytest.mark.asyncio
async def test_openrouter_transport_preserves_explicit_reasoning_off() -> None:
    captured = {}
...
@pytest.mark.asyncio
async def test_openrouter_omits_max_tokens_when_uncapped() -> None:
    captured = {}
    ...
    response: Any = await transport.complete(request)
    await client.aclose()
...
@pytest.mark.asyncio
async def test_openrouter_retries_retryable_provider_failure() -> None:
    attempts = 0
    payloads = []
