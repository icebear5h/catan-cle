...
def _ok(message: dict[str, Any] | None = None, **extra: object) -> httpx.Response:
...
@pytest.mark.asyncio
async def test_cerebras_transport_sends_native_reasoning_and_keeps_evidence() -> None:
    captured = {}
...
@pytest.mark.asyncio
async def test_cerebras_maps_every_harness_effort(reasoning: dict[str, bool] | dict[str, str] | None, expected: str) -> None:
    captured = {}
...
def test_cerebras_rejects_reasoning_token_budget() -> None:
...
@pytest.mark.asyncio
async def test_cerebras_retries_rate_limit_then_succeeds() -> None:
    attempts = 0
...
@pytest.mark.asyncio
async def test_cerebras_does_not_retry_client_errors() -> None:
    attempts = 0
