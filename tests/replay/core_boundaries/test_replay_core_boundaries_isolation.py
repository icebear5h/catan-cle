"""Replay contexts, revisions, and core runtime stay free of viewer shims."""
...
def test_replay_context_uses_the_same_general_suite_as_live_games() -> None:
    game = GameEngine(
...
def test_replay_view_reports_replay_revision_not_engine_action_count() -> None:
    game = GameEngine(
...
def test_replay_stale_check_uses_owned_engine_not_removed_state_alias() -> None:
    game = GameEngine(
        [Color.RED, Color.BLUE, Color.WHITE, Color.ORANGE],
        seed=3,
        shuffle_players=False,
...
def test_legacy_viewer_core_shims_are_removed() -> None:
    legacy_paths = (
...
def test_core_runtime_has_no_viewer_or_web_framework_imports() -> None:
    forbidden = (
