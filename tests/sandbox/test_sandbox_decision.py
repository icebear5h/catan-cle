...
def test_default_decision_menu_belongs_to_current_actor() -> None:
...
def test_off_turn_context_does_not_expose_private_development_card_menu() -> None:
    engine = GameEngine(COLORS, seed=7, shuffle_players=False)
...
def test_explicit_empty_menu_does_not_fall_back_to_engine_menu() -> None:
    engine = GameEngine(COLORS, seed=7, shuffle_players=False)
...
def test_mismatched_action_actor_is_rejected() -> None:
    engine = GameEngine(COLORS, seed=7, shuffle_players=False)
...
def test_off_turn_trade_context_uses_only_explicit_actor_owned_menu() -> None:
    engine = GameEngine(COLORS, seed=7, shuffle_players=False)
...
def test_terminal_guard_precedes_even_an_explicit_advertised_menu(override: bool) -> None:
    engine = GameEngine(COLORS, seed=7, shuffle_players=False)
...
def test_terminal_preview_opt_in_restores_only_current_actors_exact_menu(actor: Color) -> None:
    engine = GameEngine(COLORS, seed=7, shuffle_players=False)
...
def test_terminal_preview_opt_in_does_not_reveal_offturn_private_menu() -> None:
    engine = GameEngine(COLORS, seed=7, shuffle_players=False)
    state = engine.state
...
def test_terminal_preview_flag_requires_an_explicit_boolean(flag: int | str | None) -> None:
...
def test_terminal_preview_does_not_default_an_invalid_actor_to_current_player(actor: str | bool | Color) -> None:
    engine = GameEngine(COLORS, seed=7, shuffle_players=False)
...
def test_decision_social_facts_are_projected_bounded_and_detached(
    actor: Color, overflow: bool
) -> None:
    engine: Any = GameEngine(COLORS, seed=7, shuffle_players=False)
    ...
    context: Any = build_decision_context(
...
def test_decision_discard_count_uses_the_actors_own_hand() -> None:
    engine = GameEngine(COLORS, seed=7, shuffle_players=False)
