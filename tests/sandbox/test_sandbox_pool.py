...
@pytest.mark.asyncio
async def test_pool_runs_at_least_64_sandboxes_concurrently() -> None:
    active = 0
    peak = 0
...
@pytest.mark.asyncio
async def test_pool_rejects_duplicate_sandbox_in_one_batch() -> None:
...
@pytest.mark.asyncio
async def test_pool_reserves_identities_before_scheduling_children() -> None:
...
async def test_pool_failure_cleans_up_siblings_before_releasing_reservations(cancel: bool) -> None:
    entered = asyncio.Event()
    never = asyncio.Event()
...
@pytest.mark.asyncio
async def test_pool_cancellation_releases_semaphore_waiters_and_reservations() -> None:
    entered = asyncio.Event()
    never = asyncio.Event()
    calls = []
