...
from scripts.board_bench.builders.build_catan_board_bench_piece_visuals import (
    DEFAULT_CONTRACT,
    DEFAULT_OUTPUT_DIR,
    DatasetWriter,
)
...
from sft.scripts.builders.build_vlm_sft_dataset import (
    DEFAULT_EXCLUDE,
...
from sft.scripts.builders.convert_to_qwen_series_sft import convert_file
from sft.scripts.render.render_contract_images import message_view


def test_sft_layout_migration_receipt() -> None:
...
def test_canonical_sft_paths_are_outside_source_package() -> None:
...
def test_piece_writer_supports_a_fresh_output_directory(tmp_path: Path) -> None:
...
def test_resolve_dataset_asset_prefers_dataset_relative_path() -> None:
...
def test_missing_leakage_ledger_fails_closed(tmp_path: Path) -> None:
    with pytest.raises(FileNotFoundError, match="Held-out game-ID ledger"):
        load_excluded_game_ids(tmp_path / "missing.json")


def test_default_leakage_ledger_is_populated() -> None:
...
def test_invalid_leakage_ledger_schema_fails_closed(tmp_path: Path) -> None:
...
def test_vlm_row_uses_repository_relative_image_path() -> None:
...
def test_qwen_conversion_resolves_relative_images(tmp_path: Path) -> None:
...
def test_rendered_message_rows_keep_relative_image_paths() -> None:
    row = {
        "id": "row-1",
        "sample_id": "sample-1",
