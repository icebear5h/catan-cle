...
from sft.scripts.eval.failure_scorecard import eval_jsonl_for, is_synthetic, main, readout_skips
...
def test_counts_one_planted_miss_per_mode(tmp_path: Path, fixture_contract: dict, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    counts = entry["counts"]
    for key in ("road_recall", "settlement_recall", "city_recall", "empty_precision", "readout_occupied_item_recall", "tile_resource_recall", "tile_number_recall", "port_recall"):
        counts.pop(key)
    assert counts == {
        "rows": 6,
        "errors": 5,
...
def test_baseline_deltas_are_reported(tmp_path: Path, fixture_contract: dict, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_row_entanglement_uses_the_inspector_when_available(tmp_path: Path, fixture_contract: dict, monkeypatch: pytest.MonkeyPatch) -> None:
    calls = []

    def fake_inspect(
        path: Path,
        tokens: object,
        twin_threshold: float = 0.25,
        family_floor: float = 0.05,
    ) -> dict[str, object]:
...
def test_eval_jsonl_for_inverts_set_ids(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
