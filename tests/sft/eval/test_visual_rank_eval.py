...
from sft.analysis.visual_rank_eval import (
    paired_summary,
    reconstruct_weight,
    select_terrain_boards,
    strict_readout_score,
)


def board(layout: object, state: object) -> dict[str, object]:
    rows = []
...
def test_selection_is_order_independent_and_covers_all_tasks() -> None:
...
def test_selection_rejects_duplicates_missing_heads_and_training_rows() -> None:
    rows = board("a", "0")
...
def test_reconstruction_is_base_plus_delta_not_trained_plus_delta() -> None:
    base = torch.eye(3)
...
def test_reconstruction_keeps_vectors_and_handles_flattened_convolution() -> None:
...
def test_strict_readout_rejects_duplicates_omissions_extras_and_wrong_order() -> None:
...
def test_summary_requires_exact_row_coverage_and_scores_readouts_strictly() -> None:
    target = "<T00> wood 5; <P00> 3:1 port"
    records = [
