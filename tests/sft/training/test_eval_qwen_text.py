...
from trl_catan_text.support import (
    TinyTextVLM,
    text_row,
    text_tokenizer,
    write_text_source,
...
def forbid_media(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_text_messages_and_cli_preserve_historical_defaults() -> None:
...
def test_real_no_image_generation_matches_direct_language_forward(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, return_logits: bool) -> None:
    ...
    def factory(model_id: str, **kwargs: object) -> torch.nn.Module:
    ...
    def inspect_inputs(
        module: torch.nn.Module, args: tuple[object, ...], kwargs: dict[str, torch.Tensor]
    ) -> None:
...
def test_text_eval_writes_scores_summaries_without_resolving_images(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_text_eval_rejects_media_before_generation_or_path_resolution(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, extra: dict[str, str] | dict[str, object]) -> None:
    forbid_media(monkeypatch)
...
def test_generation_budget_counts_native_header_and_reserved_output(monkeypatch: pytest.MonkeyPatch) -> None:
    forbid_media(monkeypatch)
...
def test_input_mode_does_not_reinterpret_existing_task_scores() -> None:
...
def test_text_eval_entrypoint_enforces_pinned_runtime_before_model_load(monkeypatch: pytest.MonkeyPatch) -> None:
    args = evaluator.parse_args([
        "--eval-jsonl", "e", "--output-dir", "o", "--input-mode", "text",
...
def test_text_model_load_requires_a_real_checkpoint_before_loading() -> None:
