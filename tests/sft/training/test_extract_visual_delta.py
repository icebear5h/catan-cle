...
from sft.scripts.report.extract_visual_delta import (
...
def test_key_mapping_does_not_require_same_peft_prefix() -> None:
...
def test_base_cast_matches_trainer_and_prevents_false_delta() -> None:
...
def test_nonfinite_and_shapes_rejected() -> None:
...
def test_dense_fp64_delta_survives_cancellation() -> None:
...
def test_extreme_exponent_gap_is_not_claimed_lossless() -> None:
...
def test_svd_factor_orientation_energy_and_bases() -> None:
...
def test_zero_matrix_and_non_lora_tensors() -> None:
...
def fixture_sources(tmp_path: Path) -> tuple[Path, Path]:
...
def test_end_to_end_exact_dense_delta_and_no_source_mutation(tmp_path: Path) -> None:
...
def test_provenance_and_checkpoint_precision_validation(tmp_path: Path) -> None:
    args = fixture_sources(tmp_path)
