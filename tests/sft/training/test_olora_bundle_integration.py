...
class _LanguageContainer(torch.nn.Module):
    ...
    @property
    def embed_tokens(self) -> torch.nn.Embedding:
...
def test_frozen_parent_initialization_update_and_reload(
    tmp_path: Path, base_dtype: torch.dtype
) -> None:
    ...
    def logits(target: torch.nn.Module) -> torch.Tensor:
