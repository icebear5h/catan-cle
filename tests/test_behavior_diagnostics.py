...
def test_behavior_names_separate_pair_single_tile_and_full_board_heads():
    pair = _record(
        metadata={
...
def test_summarize_behaviors_carries_candidate_accuracy():
    record = _record(
        metadata={
            "task_family": "single_piece_localization",
            "task_type": "occupancy_positive",
...
def test_behavior_history_computes_best_so_far_minus_current():
...
def test_behavior_history_marks_missing_and_rejects_noncomparable_rows():
    checkpoints = [
        {
            "label": "before",
            "cells": {
...
def test_behavior_history_aligns_historical_set_names_by_exact_row_fingerprint():
    checkpoints = [
        {
            "label": "v3",
            "cells": {
...
def test_eval_set_fingerprint_keeps_shared_behavior_subsets_distinct():
    checkpoint = {
        "label": "one",
        "cells": {
            "v2/single.positive": {
...
def test_load_checkpoint_results_reads_only_original_variant(tmp_path):
