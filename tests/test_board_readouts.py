...
from sft.board_readouts import (
    NONE_ANSWER,
    connected_roads,
...
def test_render_and_parse_round_trip(partition):
    assert parse_answer(render_answer(partition)) == partition


def test_empty_partition_renders_as_none():
    assert render_answer([]) == NONE_ANSWER
    assert parse_answer(NONE_ANSWER) == []


def test_perfect_answer_scores_exact(partition):
...
def test_component_order_and_edge_order_do_not_matter(partition):
...
def test_merging_all_components_keeps_edges_but_loses_structure(partition):
...
def test_splitting_a_component_is_penalised_by_pair_agreement(partition):
...
def test_dropped_edge_shows_as_missing(partition):
...
def test_spurious_edge_shows_as_spurious(partition):
...
def test_none_answer_against_real_partition_scores_zero_recall(partition):
...
def test_pair_agreement_degrades_smoothly(partition):
...
def test_implied_helpers_agree_with_the_partition(partition):
...
def test_readout_implies_existing_component_golds():
    from sft.scripts import build_board_fluency_review as rev
    from sft.symbolic_board_tasks import _atlas, decode_state
...
def test_every_owned_road_appears_exactly_once():
    """A partition, not a cover: no edge may repeat across components."""
    from sft.scripts import build_board_fluency_review as rev
    from sft.symbolic_board_tasks import _atlas
