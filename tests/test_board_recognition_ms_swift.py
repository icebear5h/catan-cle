...
def test_ms_swift_row_uses_standard_messages_images_and_semantic_text():
...
def test_semantic_audit_preserves_source_tokens_only_as_provenance():
    audit = semantic_audit_row(plan(), query(), state=state())
...
def test_semantic_rows_and_audits_pass_draft_2020_12_schemas():
...
def test_semantic_row_rejects_opaque_targets_and_escaping_images():
    opaque = ms_swift_row(query(), image_name="state.png")
...
def test_semantic_export_refuses_historical_projection_path(tmp_path: Path):
