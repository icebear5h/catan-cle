...
def _fixture_state(sample_id: str):
...
def test_readouts_walk_every_token_in_order_with_explicit_empties():
...
def test_full_coverage_rows_match_the_semantic_contract():
    ...
    edges = {edge["token"]: edge for edge in contract["edges"]}
    for row in short:
        token = row["target_token"]
        if row["entity_type"] == "node":
            node = nodes[token]
            class_name = "EMPTY" if node["building"] is None else f"{node['color']}_{node['building']}"
...
def test_export_splits_by_layout_with_full_coverage_evals():
    output = FIXTURE_ROOT / "node_edge_readout_test_output"
    _remove_tree(output)
    try:
        metadata = export_node_edge_readout(FIXTURE_ROOT, output_dir=output, overwrite=True, validate_dataset=False)
