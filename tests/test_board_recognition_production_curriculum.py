...
def test_four_to_one_interleave_is_deterministic_and_lossless():
    base = _records("base", 12)
    supplement = _records("supplement", 3)

    first = interleave_four_to_one(base, supplement, stage="clean_board_grounding")
...
def test_four_to_one_interleave_rejects_distribution_drift():
...
def test_padding_needed_aligns_stage_boundaries(rows: int, expected: int):
    assert padding_needed(rows) == expected


def test_padding_needed_accepts_an_explicit_alignment():
    assert padding_needed(1_032, multiple=8) == 0


def test_parse_stage_list_orders_and_rejects():
