...
def test_game_split_is_deterministic_complete_and_disjoint():
    sources = [{"game_id": str(1000 + index)} for index in range(53)]
...
def test_state_selection_caps_trajectories_and_spans_density_bins():
    rows = [
...
def test_dense_labels_are_complete_without_stage_or_counterfactual_fields():
    engine = GameEngine(COLORS, seed=12, shuffle_players=False)
    contract = CatanObservationSuite().public_board_contract(engine)

    labels = dense_labels(contract, sample_id="engine-state")
...
def test_engine_trajectory_uses_unique_legal_board_changes():
...
def test_engine_trajectory_is_stable_across_process_hash_seeds():
    script = """
import json
...
def test_v2_schemas_are_valid_json_documents():
