...
def test_rows_mirror_production_forward_prompts():
    ...
    negatives = [row for row in rows if row["polarity"] == "hard_negative"]
...
def test_build_board_renders_one_image_per_placement(tmp_path):
    ...
    for row in rows:
        ...
        hops = neighbor_distances(neighbors, target).get(queried)
...
def test_neighbor_tokens_follow_the_contract_graph_and_empty_sampling_splits_them():
    _, contract = _fixture_contract()
    neighbors = neighbor_tokens(contract)
