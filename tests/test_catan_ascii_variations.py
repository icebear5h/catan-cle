...
from scripts.eval_catan_board_bench_ascii_variations import (
    build_jobs,
    build_plan,
...
def test_openrouter_result_fails_closed_on_model_and_reasoning_fields(
    ...
    monkeypatch.setattr(
        "scripts.eval_catan_board_bench_ascii_variations.httpx.Client",
        FakeClient,
    )
...
def test_resume_rejects_stale_prompt_provider_and_reasoning() -> None:
    ...
    stale_prompt = dict(record, prompt_sha256="wrong")
    wrong_provider = dict(record, provider="Chutes")
    hidden_reasoning = dict(record, native_reasoning="private")
    ...
    rejected = response_record(job, dict(result, provider="Chutes"), plan)
    assert "unexpected provider" in rejected["error"]
    missing_model = response_record(job, dict(result, served_model=None), plan)
    assert "unexpected served model" in missing_model["error"]
    reasoning_content = response_record(
        job,
...
def test_plan_hash_commits_expected_answers(tmp_path: Path) -> None:
    build_ascii_variation_dataset(tmp_path, contract_dir=CONTRACT_DIR)
    questions = [json.loads(line) for line in (tmp_path / "qa.jsonl").read_text().splitlines()][:1]
    jobs = build_jobs(
        tmp_path,
        questions=questions,
