...
from scripts.build_catan_board_recognition_curriculum import (
    DEFAULT_SPEC_PATH,
    build_dataset,
...
def test_curriculum_spec_uses_dense_raw_board_contract():
...
def test_builder_emits_balanced_dense_counterfactual_pilot(tmp_path: Path):
...
def test_build_is_deterministic_except_metadata_timestamp(tmp_path: Path):
...
def test_validator_rejects_a_second_counterfactual_label_change(tmp_path: Path):
    output_dir = tmp_path / "curriculum"
    build_dataset(output_dir=output_dir, image_size=64, seed=2026)
    rows = read_jsonl(output_dir / "manifest.jsonl")
    changed = next(row for row in rows if row["counterfactual_role"] == "counterfactual")
    label_path = output_dir / changed["label_path"]
...
def test_validator_fails_closed_on_benchmark_game_id(tmp_path: Path):
    output_dir = tmp_path / "curriculum"
...
def test_dense_diff_names_the_single_symbolic_field():
    first = {
