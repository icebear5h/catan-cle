...
from playground.game_viewer.commentary.contextualizer import CausalCommentarySession
...
def _reveal(session, context, provisional=None):
    token = session.commit(context.context_id, provisional or {})
    with contextlib.redirect_stdout(io.StringIO()):
        return session.reveal_one(token)


def _ids_containing(context, phrase):
    return [span.evidence_id for span in context.commentary if phrase in span.text]


def test_opening_semantic_episodes_overlap_and_share_confirming_event(paired_session):
...
def test_episode_can_span_events_and_preserve_observer_attribution(paired_session):
    session, workspace = paired_session
    opening = session.begin()
    workspace.ingest_context(opening)
...
def test_workspace_is_atomic_read_only_and_game_scoped(paired_session):
    session, workspace = paired_session
    ...
    snapshot = workspace.add_evidence("atomic", [evidence_ids[0]])
