...
def test_real_pre_knight_player_choice_pickle_preserves_discard_and_defaults_destination():
...
def test_current_player_choice_pickle_preserves_knight_destination():
    choice = PlayerChoice(1, knight_destination=(1, -1, 0))
    assert pickle.loads(pickle.dumps(choice)) == choice
    with pytest.raises(FrozenInstanceError):
