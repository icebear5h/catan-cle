...
from sft.board_fluency_scoring import TRANSPORT_TOKENS
from sft.coordinate_comparison import (
    ATLAS_ATOM, COORDINATE_ATOM, DEFAULT_SOURCE, INCIDENCE_RELATIONS, PAIR_QUOTAS,
    REPRESENTATIONS, SCHEMA, build_comparison_rows, coordinate_mapping, load_source_rows,
    mapping_artifact, mapping_sha256, paired_summary, project_text, read_jsonl,
    score_coordinate_comparison, select_source_cases, validate_comparison_rows,
)
from sft.scripts.build_coordinate_comparison import build_dataset
from sft.scripts import eval_qwen_vl_adapter as evaluator
from sft.symbolic_board_tasks import atlas_geometry, symbolic_answer, symbolic_prompt
...
def test_all_154_mapping_points_are_exact_integer_reversible_identities():
...
def test_complete_panel_roundtrips_source_pair_contract_and_no_prompt_leakage(rows, sources):
...
def test_missing_quotas_and_mismatched_pair_provenance_prompt_and_gold_fail(rows, sources):
...
def test_strict_answers_and_trailing_versus_interior_transport_tokens(rows):
...
def test_paired_summary_rescores_raw_outputs_and_requires_two_matching_arms(rows):
...
def test_native_evaluator_dispatches_and_summarizes_both_representations(rows):
    records = []
    for row in rows:
...
def test_builder_writes_inspectable_immutable_artifacts(tmp_path, rows):
