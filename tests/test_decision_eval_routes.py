...
def test_decision_eval_catalog_exposes_authored_buckets_labels_and_rubrics():
...
def test_decision_eval_list_filters_real_bucketed_decisions_without_mutating_replay():
    sentinel = object()
    prior_sandbox = server_state.current_sandbox
...
def test_decision_eval_detail_separates_legacy_rationale_from_native_reasoning():
    response = app.test_client().get(
...
def test_decision_eval_invalid_run_filter_and_decision_are_bounded():
...
def test_board_eval_comparison_tolerates_null_optional_metrics(monkeypatch):
    monkeypatch.setattr(
        bench,
...
def test_real_decision_eval_artifact_covers_manifest_and_zero_count_buckets():
