...
from sft.board_state_readout import board_keys, parse_state, score_board_state, summarize_board_states
from sft.scripts.eval_qwen_vl_adapter import is_long_answer, score_response
...
def test_every_board_location_and_robber_match_source_contract():
...
def test_duplicates_truncation_extras_and_order_are_not_exact():
    _, contract = fixture()
...
def test_all_empty_prediction_cannot_hide_missing_pieces():
    state, contract = fixture()
    target = board_answer(contract)
...
def test_full_board_training_guard_rejects_incomplete_targets():
    state, contract = fixture()
    row = row_for_state(state, contract)
...
def test_validation_sample_covers_occupied_boards_in_every_layout():
...
def test_layout_macro_does_not_overweight_one_layout():
    _, contract = fixture()
