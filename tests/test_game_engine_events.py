...
def test_engine_step_appends_one_canonical_event_and_projects_it():
    engine = _engine()
...
def test_private_event_overlays_never_enter_other_player_projection():
    bought = event_from_action(
        Action(Color.RED, ActionType.BUY_DEVELOPMENT_CARD, "KNIGHT"),
        0,
    )
    stolen = event_from_action(
        Action(Color.RED, ActionType.STEAL, (Color.BLUE, "ORE")),
        1,
    )
    discarded = event_from_action(
...
def test_trade_events_expose_named_semantic_payloads():
...
def test_engine_observation_exposes_legal_actions_only_to_current_player():
    engine = _engine()
...
def test_snapshot_restore_includes_rng_state_and_event_log():
    engine = _engine(23)
...
def test_live_step_does_not_copy_full_state(monkeypatch):
    engine = _engine()
