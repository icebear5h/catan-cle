...
from scripts.eval_catan_initial_settlement_reasoning import (
    build_seed_input,
    capture_trace,
    catalog_cost_bounds,
    load_prompt_variant,
...
def test_fresh_initial_settlement_input_is_deterministic_and_history_free():
    ...
    model_prompt = first.manifest["messages"][-1]["content"]
...
def test_uncapped_cost_bounds_use_catalog_max_completion_and_prompt_allowance():
    catalog = {
        "models": [
            {
                "model_id": "model/a",
...
@pytest.mark.asyncio
async def test_capture_trace_omits_max_tokens_and_separates_native_reasoning():
    captured = {}
