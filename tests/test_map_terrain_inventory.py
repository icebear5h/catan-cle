...
def test_standard_template_has_the_19_hex_terrain_inventory():
...
def test_random_boards_preserve_terrain_counts_across_seeds():
    layouts = set()
...
def test_explicit_terrain_order_is_preserved_without_consuming_the_input():
...
def test_explicit_terrain_order_rejects_invalid_inventory(invalid_kind):
...
def test_template_inventory_must_match_the_number_of_land_positions(size_delta):
    resources = BASE_MAP_TEMPLATE.tile_resources.copy()
    if size_delta < 0:
        resources.pop(0)
    else:
...
def test_mini_map_keeps_its_own_inventory():
...
def test_tournament_map_preserves_its_layout_and_standard_inventory():
