...
class FakeModel(torch.nn.Module):
    ...
    def resize_token_embeddings(self, size, *, pad_to_multiple_of, mean_resizing):
...
def test_semantic_inventory_loader_requires_exact_bidirectional_contract(tmp_path):
...
def test_prepare_semantic_tokens_adds_regular_atomic_rows_and_freezes_base_output():
...
def test_input_embedding_module_name_is_exact_and_unique():
    model = FakeModel()
...
def test_prepare_semantic_tokens_rejects_tied_or_partial_tokenizers():
...
def test_scope_audit_accepts_only_visual_aligner_atlas_rows_and_language_lora(tmp_path):
    model = AuditModel()
...
def test_scope_audit_rejects_base_language_and_output_updates(tmp_path):
    model = AuditModel()
...
def test_optimizer_coverage_requires_every_trainable_exactly_once(tmp_path):
    model = AuditModel()
