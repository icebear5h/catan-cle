...
def response_case(request, oversized_integer):
    kind = request.param
    ...
    if kind.startswith("trade_"):
        ...
        payload = {"give": {"WOOD": 1}, "receive": {"ORE": 1}}
        valid = f"<action>{index}</action><trade_offer>{json.dumps(payload)}</trade_offer>"
        field_name = kind.removeprefix("trade_")
        if field_name in {"give", "receive"}:
            resource = "WOOD" if field_name == "give" else "ORE"
...
def test_oversized_numeric_fields_raise_controlled_parse_errors(response_case):
...
@pytest.mark.asyncio
async def test_numeric_rejection_retries_once_and_traces_first_validation(response_case):
...
@pytest.mark.asyncio
async def test_numeric_rejection_respects_retry_budget_and_leaves_second_response_queued(response_case):
    sandbox, player, transport, invalid, valid = response_case
...
def test_json_callback_errors_are_not_normalized_as_numeric_rejections(
    response_case, error_type, monkeypatch
):
    sandbox, player, _, _, valid = response_case
    error = error_type("unrelated object callback failure")
    original_loads = json.loads

    def fail_object(pairs):
        raise error

    def decode_with_callback(text, **kwargs):
...
def test_oversized_communication_commitment_turn_fails_closed(oversized_integer):
    prefix = (
    ...
    valid = parse_communication_response(
...
@pytest.mark.parametrize("error_type", [ValueError, RuntimeError])
def test_communication_commitment_callback_errors_propagate(error_type, monkeypatch):
    error = error_type("unrelated commitment callback failure")

    def fail_commitment(*args):
        raise error
