...
def _engine_with_mutable_history():
...
def test_state_copy_detaches_historical_trade_offers_and_list_dice(mutate_original):
...
def test_state_copy_detaches_mutable_playable_payloads_including_nested_tuples():
    original = _engine_with_mutable_history().state
...
def test_state_copy_keeps_static_map_shared_without_copying_all_state(monkeypatch):
    original = _engine_with_mutable_history().state

    def reject_map_deepcopy(self, memo):
...
def test_replay_checkpoint_reuse_detaches_list_dice_and_historical_offers():
    engine = _engine_with_mutable_history()
