...
from cle.harness.models import ModelResponse, PlayerSession
...
def test_json_keeps_trained_token_and_provider_metadata(context):
    target = context.legal_actions[0]
    text = json.dumps({
    ...
    choice = PlayerResponseParser(load_context_suite()).parse(context, response)
...
def test_invalid_envelope_never_falls_back_or_mutates_context(context, text):
...
def test_default_packet_advertises_semantic_tools_and_preserves_token_identity(context):
...
@pytest.mark.asyncio
async def test_json_knight_retry_preserves_state_then_commits_one_decision():
...
def test_missing_arguments_on_a_no_parameter_tool_is_accepted(text):
    ...
    end_turn = next(i for i, a in enumerate(context.legal_actions) if a.action_type == ActionType.END_TURN)
    choice = PlayerResponseParser(load_context_suite()).parse(context, ModelResponse(content=text))
    assert choice.action_index == end_turn


def test_missing_arguments_on_a_parameter_tool_still_fails_on_its_own_terms(context):
    with pytest.raises(PlayerResponseParseError):
...
def test_stray_top_level_keys_are_named():
