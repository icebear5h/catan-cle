...
def _game(catan_map: CatanMap | None) -> GameEngine:
    return GameEngine(
        [
            Color.BLUE,
            Color.BLACK,
...
def test_extracts_hyphenated_spaced_compact_and_asr_eight_mentions() -> None:
...
def test_rejects_singles_roll_percentages_and_port_ratios() -> None:
    text = "Roll a 10. Trade at 3-1. I am 110% sure; game 242781000."

    assert extract_number_mentions(text) == ()


def test_resolves_known_pilot_corners_and_preserves_ambiguity() -> None:
...
def test_number_order_is_irrelevant_but_source_order_is_preserved() -> None:
    corner_index = build_corner_index(_paired_map())
...
def test_pair_direction_remains_a_candidate_set_not_a_guess() -> None:
    corner_index = build_corner_index(_paired_map())
...
def test_dynamic_candidate_facts_track_legality_and_occupancy() -> None:
    catan_map: Any = _paired_map()
