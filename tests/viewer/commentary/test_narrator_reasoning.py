...
def _paired_transcript() -> dict[str, Any]:
    return {
...
def _write_artifact(
    tmp_path: Path, *, include_result: bool = True, paragraph_end: float = 12.0
) -> dict[str, Any]:
...
def _load(tmp_path: Path, transcript: dict[str, Any]) -> dict[str, Any]:
    artifact: dict[str, Any] = load_narrator_reasoning_artifact(
        tmp_path,
...
def test_transcript_fingerprint_includes_alignment_policy() -> None:
    transcript = _paired_transcript()
...
def test_reasoning_artifact_loads_strict_causal_observation_group(tmp_path: Path) -> None:
    ...
    ready: Any = build_paired_narrator_reasoning_window(replay_data, 0)
...
def test_reasoning_history_never_exposes_future_observation_group(tmp_path: Path) -> None:
    transcript = _write_artifact(tmp_path)
    collection: Any = _load(tmp_path, transcript)
    future_paragraph = {
    ...
    before_future: Any = build_paired_narrator_reasoning_window(replay_data, 1)
    at_future: Any = build_paired_narrator_reasoning_window(replay_data, 2)
...
def test_reasoning_artifact_reports_pending_planned_packet(tmp_path: Path) -> None:
...
def test_reasoning_artifact_rejects_paragraph_outside_source_packet(tmp_path: Path) -> None:
...
def test_reasoning_artifact_rejects_stale_transcript_fingerprint(tmp_path: Path) -> None:
    transcript: Any = _write_artifact(tmp_path)
...
def test_reasoning_artifact_rejects_retrospective_subject(tmp_path: Path) -> None:
    transcript = _write_artifact(tmp_path)
