...
def _trace_artifacts(root: Path) -> None:
    ...
    colors: Any = (Color.BLUE, Color.RED, Color.WHITE, Color.ORANGE)
    engine = GameEngine(colors, seed=117, shuffle_players=False)
    board_sha256: Any = snapshot_public_board(engine.observe(Color.BLUE)).facts_sha256
    _write_json(
        root / "plan.json",
        {
            "models": models,
...
def _client(root: Path, monkeypatch: pytest.MonkeyPatch) -> FlaskClient:
