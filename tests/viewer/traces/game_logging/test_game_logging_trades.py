"""Trade action logs use the named resource contract and engine order."""
...
from .support import _offer


def test_trade_action_display_uses_named_resource_contract() -> None:
    action: Any = Action(Color.RED, ActionType.OFFER_TRADE, _offer())
...
def test_counteroffer_display_retains_parent_and_audience() -> None:
    action = Action(
        Color.RED,
...
def test_legacy_trade_log_is_projected_from_structured_public_event() -> None:
    public_state: Any = {
        "game_log": [
            {
                "type": "trade",
    ...
    normalized: Any = normalize_public_state_game_log(public_state)
...
def test_missing_batched_trade_responses_are_restored_in_engine_order() -> None:
...
def test_transition_batch_logs_every_trade_response() -> None:
    transitions: Any = tuple(
        SimpleNamespace(
